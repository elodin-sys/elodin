...
- **Just task runner** — `just install [py|editor|db|tracy|all]` builds and installs the Python SDK wheel (uv + maturin 1.12.6), `elodin` editor binary, and `elodin-db` binary into `$ELODIN_SHELL_BIN`; `just install tracy` builds all with Tracy profiling enabled (Linux only).
- **uv-based Python management** — All Python work uses `uv` inside the Nix shell; `just install py` installs into the nix-shell venv (Python 3.13).
...
- **Panels** — `viewport`, `graph`, `component_monitor` (live scalar values), `geo_position_gauge` (position in source frame shown as ECEF/NED/ENU/LLA), `orientation_gauge` (attitude quaternion rendered as a 3D gimbal), `query_table` (EQL/SQL), `query_plot` (EQL/SQL; `timeseries|xy`; `refresh_interval`, `auto_refresh`), `action_pane` (named Lua actions, e.g. `send_msg` presets), `video_stream`, `sensor_view`, `log_stream` (level filters TRACE–ERROR, auto-scroll), `data_overview` (all-component sparklines), `schematic_tree`, `dashboard` (Bevy-UI flex/grid layouts with text/colors), `inspector` and `hierarchy` sidebars.
