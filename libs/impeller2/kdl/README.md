...
- `spatial_gauge`: positional `eql` (required position expression; named `eql=` also accepted); optional `name`, `source` (`ECEF`/`NED`/`ENU`; inherits from global `coordinate` if omitted, else `ENU`), `display` (`ECEF`/`NED`/`ENU`/`LLA`, default `NED`), optional `reference x y z w` child (body→`source` quaternion the attitude gimbal shows as neutral; identity when omitted).
