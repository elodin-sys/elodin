...
- `timeline`
- `window`
- Panel roots: `tabs`, `hsplit`, `vsplit`, `viewport`, `graph`, `component_monitor`, `spatial_gauge`, `action_pane`, `query_table`, `query_plot`, `inspector`, `hierarchy`, `schematic_tree`, `data_overview`, `dashboard`
...
- `spatial_gauge`: positional `eql` (required position expression; named `eql=` also accepted); optional `name`, `source` (`ECEF`/`NED`/`ENU`; inherits from global `coordinate` if omitted, else `ENU`), `display` (`ECEF`/`NED`/`ENU`/`LLA`, default `NED`).
