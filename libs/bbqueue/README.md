...
3. **Vendor in monorepo** — copy bbqueue 0.7.0 into `libs/bbqueue`, apply the usize header patch
   locally, replace all `bbq2` / git `bbqueue` deps with `path` dependencies.
...
rsync -a --delete /tmp/bbqueue/bbqueue/ libs/bbqueue/
# Re-apply the framed_*_with_header block in libs/bbqueue/src/queue.rs
```
