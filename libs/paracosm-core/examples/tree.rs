...
fn sim(mut builder: XpbdBuilder<'_>, mut assets: Assets, input: Input) {
    ...
    let rod_a = builder.entity(
        EntityBuilder::default()
            .mass(1.0)
            //.effector(|BodyPos(p)| Force(Vector3::new(0., 9.8, 0.)))
    ...
    builder.entity(
        EntityBuilder::default()
            ...
            //.effector(move || Torque(Vector3::new(0.0, 0.0, *input.0.load())))
            .effector(|BodyPos(p)| Force(p.att * Vector3::new(0., -9.8, 0.)))
            //.effector(|BodyPos(p)| Force(p.att * Vector3::new(0., -9.8, 0.)))
