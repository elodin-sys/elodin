...
| **Formula: ecef_to_ned() / …** | `pos.ecef_to_ned()`, `vel.ecef_to_ned_vector()` | Directed ENU/NED/ECEF conversion | Source and target frames are in the method name. Point forms accept 3- or 7-vecs (SQL emits transformed xyz; pose attitude is editor-runtime). Vector forms are rotation-only on 3-vecs. ECEF ↔ local needs `Context.geo_origin` (schematic `coordinate`). |
