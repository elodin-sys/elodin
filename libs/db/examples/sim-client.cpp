...
try {
    ...
    auto table = builder::vtable({
        ...
        //
        // ```
        // Reader thread received data: 32 bytes
        // 1c 00 00 00 01 02 00 00 2e fb af 2b ae 41 06 00 24 06 00 00 00 00 00 00 21 09 9d 05 11 11 81 3f
        // ```
    ...
    while (true) {
        ...
        auto table_header = PacketHeader {
            .len = 4 + sizeof(global_data),
            .ty = PacketType::TABLE,
            .packet_id = { ID[0], ID[1] },
            .request_id = 0,
        };

        global_data.time = system_clock::now().time_since_epoch() / microseconds(1);
        // sensor_data.temp = std::sin(static_cast<double>(global_data.time) / 1000000.0);

        // sock.write_all(&table_header, sizeof(table_header));
        // sock.write_all(&global_data, sizeof(global_data));
        usleep(1000);
    }
