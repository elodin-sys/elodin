...
fn format_values(values: &[f64]) -> String {
    if values.is_empty() {
        ...
        let formatted: Vec<String> = values.iter().map(|v| format!("{:8.2}", v)).collect();
        format!("[{}]", formatted.join(", "))
    } else if values.len() <= 20 {
        // Show first few and last few for moderate arrays
        let first: Vec<String> = values[..3.min(values.len())]
            .iter()
            .map(|v| format!("{:6.1}", v))
            .collect();
        let last: Vec<String> = values[values.len().saturating_sub(2)..]
            .iter()
            .map(|v| format!("{:6.1}", v))
            .collect();
        format!(
...
#[component]
pub fn TelemetryApp(props: &TelemetryAppProps, mut hooks: Hooks) -> impl Into<AnyElement<'static>> {
    let receiver = props.receiver.clone();
    // --- Shared ring buffer (no re-render on every message) ---
    let ring = hooks.use_const(|| {
        Arc::new(Mutex::new(VecDeque::<TelemetryRow>::with_capacity(
            CAPACITY,
        )))
    });
    ...
    // Telemetry consumer from channel
    {
        ...
        hooks.use_future(async move {
            while let Ok(row) = receiver.recv().await {
            ...
            TerminalEvent::Key(KeyEvent { code, kind, .. }) if kind != KeyEventKind::Release => {
                match code {
                    ...
                    KeyCode::Up | KeyCode::Char('k') => {
                        scroll_back.set(scroll_back.get().saturating_add(1))
                    }
                    KeyCode::Down | KeyCode::Char('j') => {
                        scroll_back.set(scroll_back.get().saturating_sub(1))
                    }
        ...
        if !has_real_data && q.iter().any(|r| r.is_waiting) {
            ...
            let lines = match waiting_state {
                ...
                _ => vec![
                    ...
                    String::new(),
                    "The client will automatically connect when data becomes available."
                        .to_string(),
                ],
            };
            (lines.join("\n"), 0)
            ...
            for row in q.iter().rev().take(1000) {
                // Look at last 1000 entries max
                if !row.is_waiting {
                    latest_values
                        .entry(row.component_name.clone())
                        .or_insert(row.clone());
                }
            }
            ...
            for row in latest_values.values() {
                ...
                by_category.entry(category).or_default().push(row.clone());
            }
            ...
            for category_name in &category_order {
                if let Some(rows) = by_category.get(category_name) {
                    ...
                    lines.push(category_name.to_string());
...
pub async fn run_tui(receiver: async_channel::Receiver<TelemetryRow>) -> Result<()> {
    element!(TelemetryApp(receiver)).fullscreen().await?;
    Ok(())
}
