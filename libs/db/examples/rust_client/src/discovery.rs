...
pub async fn discover_components(client: &mut Client) -> Result<HashMap<ComponentId, DiscoveredComponent>> {
    info!("Discovering components from database");
    
    // Request all metadata
    let dump_metadata = DumpMetadata;
    let metadata_resp: DumpMetadataResp = client.request(&dump_metadata)
        .await
        .context("Failed to dump metadata")?;
    
    println!("\n📊 Discovered Components:");
    println!("  Found {} components registered", metadata_resp.component_metadata.len());
    
    // Request all schemas
    let dump_schema = DumpSchema;
    let schema_resp: DumpSchemaResp = client.request(&dump_schema)
        .await
    ...
    for metadata in metadata_resp.component_metadata {
        ...
        if let Some(schema) = schema_resp.schemas.iter()
...
pub fn display_rocket_summary(components: &HashMap<ComponentId, DiscoveredComponent>) {
    ...
    if !rocket_components.is_empty() {
        ...
        println!("  {} rocket-specific components available", rocket_components.len());
        ...
        for comp in &rocket_components {
            let category = if comp.name.contains("world_") {
                "Position/Motion"
            } else if comp.name.contains("aero") || comp.name.contains("mach") || comp.name.contains("pressure") {
                "Aerodynamics"
            ...
            categories.entry(category).or_insert_with(Vec::new).push(comp);
        }
