...
In the example below, two constant masses are added as a symbolic sum; to get a numeric value, the graph is lowered to a `NoxprFn` and evaluated using IREE execution.

> **Note**: The example below is currently disabled during IREE migration. 
> Execution now happens through the Python IREE runtime.
