...
use elodin::{
    ...
    spatial::SpatialPos,
    BodyPos, BodyVel, Effect, Force, SharedNum, Time,
};
    ...
    *alt_set_point.0.load() = 2.5;
    ...
    builder.entity(
        EntityBuilder::default()
            ...
            .effector(move |BodyVel(v)| Force(-v.vel * 0.5 * v.vel.norm()))
            .effector(move |BodyPos(SpatialPos { att, .. })| {
                motor(*motor_a_rpm.0.load(), att, motor_a_pos)
            })
            .effector(move |BodyPos(SpatialPos { att, .. })| {
                motor(*motor_b_rpm.0.load(), att, motor_b_pos)
            })
            .effector(move |BodyPos(SpatialPos { att, .. })| {
                motor(*motor_c_rpm.0.load(), att, motor_c_pos)
            })
            .effector(move |BodyPos(SpatialPos { att, .. })| {
                motor(*motor_d_rpm.0.load(), att, motor_d_pos)
            })
            .sensor(
                move |BodyPos(SpatialPos { att, pos }): BodyPos, BodyVel(v), Time(time)| {
                    let (yaw, pitch, roll) = att.euler_angles();
...
fn force_at_point(r: Vector3<f64>, force: Vector3<f64>, att: UnitQuaternion<f64>) -> Effect {
    Effect {
        torque: elodin::Torque(att * r.cross(&force)),
