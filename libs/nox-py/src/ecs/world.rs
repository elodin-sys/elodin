...
use crate::ecs::{Archetype, Component};
use crate::physics::globals::{SimulationTimeStep, SystemGlobals};
use bytemuck::Pod;
use elodin_db::{ComponentSchema, MetadataExt};
use impeller2::com_de::FromComponentView;
use impeller2::{
    component::Component as ImpellerComponent,
...
#[derive(Debug, PartialEq, Eq, Default, Clone, serde::Deserialize, serde::Serialize)]
pub struct Column<B> {
    pub buffer: B,
...
impl<'a, B: 'a + AsRef<[u8]>> ColumnRef<'a, B> {
    pub fn typed_buf<T: ImpellerComponent + Pod>(&self) -> Option<&[T]> {
    ...
    pub fn typed_iter<T: ImpellerComponent + FromComponentView>(
        &self,
...
impl<'a> ColumnRef<'a, &'a mut Vec<u8>> {
    pub fn typed_buf_mut<T: ImpellerComponent + Pod>(&mut self) -> Option<&mut [T]> {
    ...
    pub fn update(&mut self, offset: usize, value: ComponentView<'_>) -> Result<(), crate::Error> {
