...
mod tests {
    ...
    #[test]
    fn prefers_environment_override() {
    ...
    #[test]
    fn uses_current_exe_when_it_is_elodin() {
        let selected = select_render_server_cmd(None, Some(PathBuf::from("/tmp/elodin")));
        assert_eq!(selected, Some(PathBuf::from("/tmp/elodin")));
    }

    #[test]
    fn ignores_non_elodin_current_exe() {
        let selected = select_render_server_cmd(None, Some(PathBuf::from("/tmp/python3")));
