...
use crate::nox_ecs::{ComponentArray, join_query, update_var};
use crate::nox_ecs::{join_many, nox::Noxpr};
use impeller2::types::ComponentId;

#[pyclass]
#[derive(Clone)]
pub struct QueryInner {
    pub query: crate::nox_ecs::Query<()>,
        ...
        let (query, metadata) = component_ids
            ...
            .try_fold((None, vec![]), |(mut query, mut metadata), res| {
                ...
                if query.is_some() {
                    ...
                    let q: crate::nox_ecs::Query<()> = a.into();
                    query = Some(q);
                }
                Ok::<_, Error>((query, metadata))
            })?;
        let query = query.ok_or(crate::nox_ecs::Error::ComponentNotFound)?;
    ...
    pub fn output(&self, builder: SystemBuilder, args: Vec<PyObject>) -> Result<PyObject, Error> {
            ...
            .zip(self.metadata.iter().map(|m| m.component_id()))
        {
            let Some((meta, index)) = builder.get_var(id) else {
                return Err(crate::nox_ecs::Error::ComponentNotFound.into());
            };
            let buffer = args
                .get(index)
                .ok_or(crate::nox_ecs::Error::ComponentNotFound)?;
