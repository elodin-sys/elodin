...
(d, k) = lqr_control_mat(
    np.array([1.0, 1.0, 1.0]), np.array([1.0, 1.0]), np.array([1.0, 1.0, 1.0])
)
...
def control(q: Query[WorldPos, WorldVel]) -> ComponentArray[Force]:
    ...
    return q.map(
        Force,
        lambda p, v: Force.from_torque(
            np.clip(v.angular() * d + p.angular().vector()[:3] * k, -0.5, 0.5)
        ),
    )
...
b = Body(
    ...
    world_vel=WorldVel.from_angular(np.array([5.0, 5.0, 5.0])),
...
w.spawn(b).metadata(EntityMetadata("OreSat"))
