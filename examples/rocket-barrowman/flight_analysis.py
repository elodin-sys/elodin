...
class FlightAnalyzer:
    ...
    def compute_all_metrics(self) -> FlightMetrics:
        ...
        stability = self._compute_stability_derivatives(
            times, aoas, sideslips, machs, dynamic_pressures,
            drag_forces, lift_forces, moments, angular_velocities
        )
        ...
        drag_coeff = np.where(
            qs_safe > 1e-3,
            2 * np.linalg.norm(drag_forces, axis=1) / (qs_safe * ref_area),
            0.0
        )
        lift_coeff = np.where(
            qs_safe > 1e-3,
            2 * np.linalg.norm(lift_forces, axis=1) / (qs_safe * ref_area),
            0.0
        )
        ...
        min_stability = float(np.min(static_margin)) if isinstance(static_margin, np.ndarray) else float(static_margin)
        # Return average static margin for summary, but keep array for time-series plots
        avg_static_margin = float(np.mean(static_margin)) if isinstance(static_margin, np.ndarray) else float(static_margin)
    ...
    def _estimate_static_margin(self) -> np.ndarray:
        ...
        try:
            ...
            try:
                cp_positions = np.array([
                    float(rocket.aero.calculate_cp(mach)) 
                    for mach in machs
                ])
            except:
    ...
    def _compute_stability_derivatives(
        ...
        valid_q = np.where((np.abs(q_normalized) > 1e-4) & (qs_safe > 1e-3) & (np.abs(aoas) < np.radians(10)))[0]
        if len(valid_q) > 10:
        ...
        valid_aoa = np.where((np.abs(aoas) > 0.01) & (qs_safe > 1e-3) & (np.abs(aoas) < np.radians(30)))[0]
        if len(valid_aoa) > 10:
        ...
        valid_alphadot = np.where((np.abs(alpha_rate_normalized) > 1e-4) & (qs_safe > 1e-3) & (np.abs(aoas) < np.radians(10)))[0]
        if len(valid_alphadot) > 10:
            C_m_alphadot = np.polyfit(alpha_rate_normalized[valid_alphadot], C_m_pitch[valid_alphadot], 1)[0]
        else:
            C_m_alphadot = -1.0  # Typical damping value
        
        # C_L_q: Lift due to pitch rate (dC_L/dq)
        valid_lift_q = np.where((np.abs(q_normalized) > 1e-4) & (qs_safe > 1e-3) & (np.abs(aoas) < np.radians(10)))[0]
        ...
        valid_beta = np.where((np.abs(sideslips) > 0.01) & (qs_safe > 1e-3) & (np.abs(sideslips) < np.radians(30)))[0]
        ...
        valid_roll_beta = np.where((np.abs(sideslips) > 0.01) & (qs_safe > 1e-3) & (np.abs(aoas) < np.radians(10)))[0]
        ...
        valid_roll_p = np.where((np.abs(p_normalized) > 1e-4) & (qs_safe > 1e-3) & (np.abs(aoas) < np.radians(10)))[0]
        ...
        valid_yaw_beta = np.where((np.abs(sideslips) > 0.01) & (qs_safe > 1e-3) & (np.abs(aoas) < np.radians(10)))[0]
        ...
        valid_yaw_r = np.where((np.abs(r_normalized) > 1e-4) & (qs_safe > 1e-3) & (np.abs(aoas) < np.radians(10)))[0]
        ...
        valid_cross = np.where((np.abs(p_normalized) > 1e-4) & (qs_safe > 1e-3) & (np.abs(aoas) < np.radians(10)))[0]
...
def compute_flight_phases(result: FlightResult, motor_burn_time: float) -> Dict[str, Tuple[int, int]]:
