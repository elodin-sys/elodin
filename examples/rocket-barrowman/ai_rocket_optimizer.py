...
class RocketOptimizer:
    ...
    MOTOR_COST_ESTIMATES = {
        ...
        "F": 30, "G": 45, "H": 70, "I": 100, "J": 150,
        "K": 200, "L": 300, "M": 450, "N": 700, "O": 1000,
    }
...
Be thorough with unit conversions. Return only valid JSON."""
            ...
            response = self.openai_client.chat.completions.create(
                model="gpt-4o-mini",
                messages=[
                    {"role": "system", "content": "You are a rocket design expert. Extract requirements precisely."},
    ...
    def _build_rocket_for_motor(
        ...
        body_radius = min(standard_radii, key=lambda r: abs(r - body_radius) if r >= motor.diameter/2 else float('inf'))
        ...
        try:
            ...
            if not result.history:
                return SimulationResult(
                    max_altitude_m=0, max_velocity_ms=0, max_mach=0,
                    max_acceleration_g=0, apogee_time_s=0, flight_time_s=0,
                    landing_distance_m=0, stability_margin=0, success=False,
    ...
    def _calculate_confidence(
        ...
        if sim.stability_margin < 1.0:
            ...
            warnings.append(f"Stability margin {sim.stability_margin:.2f} cal is high (may weathercock)")
        
        # Landing distance
        if requirements.landing_distance_m:
            if sim.landing_distance_m > requirements.landing_distance_m:
                issues.append(f"Landing {sim.landing_distance_m:.0f}m exceeds limit {requirements.landing_distance_m:.0f}m")
                score -= 15
    ...
    def _estimate_total_cost(
        ...
        fin_area = 4 * 0.5 * (config.get("fin_root_chord", 0.1) + config.get("fin_tip_chord", 0.05)) * config.get("fin_span", 0.1)
    ...
    def optimize(
        ...
        for i, motor in enumerate(motor_candidates[:max_simulations]):
            ...
            log.append(f"  Impulse: {motor.total_impulse:.0f} N·s, Cost: ${motor.estimated_cost_usd:.0f}")
            ...
            if sim.success:
                log.append(f"  Altitude: {sim.max_altitude_m:.0f} m ({sim.max_altitude_m * 3.281:.0f} ft)")
        ...
        if working:
            ...
            if target:
                most_accurate = min(working, key=lambda c: abs(c.simulation.max_altitude_m - target))
                log.append(f"Most accurate: {most_accurate.motor.designation} ({most_accurate.simulation.max_altitude_m:.0f}m)")
