...
class ThrustCurveScraper:
    ...
    def __init__(self, cache_dir: Optional[Path] = None):
        ...
        self.session.headers.update({
            'User-Agent': 'Elodin-Rocket-Simulator/1.0',
            ...
            if 'results' in data:
                for motor in data['results']:
                    motors.append({
                        ...
                        'manufacturer': motor.get('manufacturer', {}).get('name', 'Unknown') if isinstance(motor.get('manufacturer'), dict) else str(motor.get('manufacturer', 'Unknown')),
    ...
    def get_motor_data(self, motor_id: str, search_result: Optional[Dict] = None) -> Optional[MotorData]:
        """
        ...
        try:
            ...
            total_impulse = calculated_total_impulse if calculated_total_impulse > 0 else motor_info.get('totalImpulse', 0.0)
            avg_thrust = calculated_avg_thrust if calculated_avg_thrust > 0 else motor_info.get('avgThrust', 0.0)
            burn_time = calculated_burn_time if calculated_burn_time > 0 else motor_info.get('burnTime', 0.0)
            max_thrust = max([t for _, t in thrust_curve]) if thrust_curve else motor_info.get('maxThrust', 0.0)
    ...
    def _download_and_parse_data_file(self, motor_id: str, format: str = 'eng') -> List[Tuple[float, float]]:
    ...
    def scrape_motor_list(self, max_motors: int = 10000, impulse_classes: Optional[List[str]] = None) -> List[MotorData]:
        """
        ...
        if impulse_classes is None:
            ...
            impulse_classes = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J', 'K', 'L', 'M', 'N', 'O']
