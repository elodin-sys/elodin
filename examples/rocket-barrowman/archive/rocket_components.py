...
class MassProperties:
    ...
    def __add__(self, other: "MassProperties") -> "MassProperties":
        ...
        ixx = self.ixx + self.mass * (dy1**2 + dz1**2) + other.ixx + other.mass * (dy2**2 + dz2**2)

        iyy = self.iyy + self.mass * (dx1**2 + dz1**2) + other.iyy + other.mass * (dx2**2 + dz2**2)

        izz = self.izz + self.mass * (dx1**2 + dy1**2) + other.izz + other.mass * (dx2**2 + dy2**2)

        return MassProperties(
            mass=total_mass, cg_x=cg_x, cg_y=cg_y, cg_z=cg_z, ixx=ixx, iyy=iyy, izz=izz
        )
...
class RocketComponent(ABC):
    ...
    @abstractmethod
    def calculate_own_aerodynamic_properties(
        self, mach: float, body_diameter: float
    ...
    def get_aerodynamic_properties(
        self, mach: float, body_diameter: float
    ) -> AerodynamicProperties:
        ...
        return AerodynamicProperties(
            cn_alpha=total_cn_alpha, cp_x=cp_x, cd_base=total_cd, cd_friction=0.0
        )
...
class NoseCone(RocketComponent):
    ...
    def calculate_own_mass_properties(self) -> MassProperties:
        ...
        if self.shape == NoseShape.CONICAL:
            ...
            inner_volume = (
                (1 / 3) * math.pi * (self.diameter / 2 - self.thickness) ** 2 * self.length
            )
        ...
        return MassProperties(mass=mass, cg_x=cg_x, ixx=ixx, iyy=iyy, izz=izz)
    ...
    def calculate_own_aerodynamic_properties(
        ...
        return AerodynamicProperties(
            cn_alpha=cn_alpha, cp_x=cp_x, cd_base=cd_base, cd_friction=cd_friction
        )
...
class BodyTube(RocketComponent):
    ...
    def calculate_own_aerodynamic_properties(
        ...
        return AerodynamicProperties(
            cn_alpha=cn_alpha, cp_x=cp_x, cd_base=0.0, cd_friction=cd_friction
        )
...
class FinSet(RocketComponent):
    ...
    def calculate_own_mass_properties(self) -> MassProperties:
        ...
        return MassProperties(mass=total_mass, cg_x=cg_x, ixx=ixx, iyy=iyy, izz=izz)
    ...
    def calculate_own_aerodynamic_properties(
        ...
        return AerodynamicProperties(
            cn_alpha=cn_alpha, cp_x=cp_x, cd_base=cd_base + cd_interference, cd_friction=0.0
        )
...
class Parachute(RocketComponent):
    ...
    def calculate_own_mass_properties(self) -> MassProperties:
        ...
        return MassProperties(
            mass=self.packed_mass, cg_x=self.position_x, ixx=0.0, iyy=0.0, izz=0.0
        )
    ...
    def calculate_own_aerodynamic_properties(
        ...
        if self.deployed:
            ...
            return AerodynamicProperties(
                cn_alpha=0.0, cp_x=self.position_x, cd_base=cd_total, cd_friction=0.0
            )
        else:
            # Packed parachute (negligible drag)
            return AerodynamicProperties(
                cn_alpha=0.0, cp_x=self.position_x, cd_base=0.0, cd_friction=0.0
            )
