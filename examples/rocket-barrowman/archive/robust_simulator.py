...
class RobustSimulator:
    ...
    def compute_aerodynamic_forces_and_moments(
        self, state: State, mass: float, cg: float, rho: float, a: float
        ...
        F_body = np.array([F_axial, F_normal_yaw, F_normal_pitch])
        ...
        M_pitch_damping = (
            C_mq * (state.wy * c_ref / (2 * v_mag)) * q * S_ref * c_ref if v_mag > 1 else 0
        )
        M_yaw_damping = (
            C_nr * (state.wz * c_ref / (2 * v_mag)) * q * S_ref * c_ref if v_mag > 1 else 0
        )
    ...
    def state_derivative(self, state: State, t: float) -> np.ndarray:
        ...
        deriv = np.array(
            [
                state.vx,
                state.vy,
                state.vz,  # Position derivatives
                accel_world[0],
                accel_world[1],
                accel_world[2],  # Velocity derivatives
                q_dot[0],
                q_dot[1],
                q_dot[2],
                q_dot[3],  # Quaternion derivatives
                alpha_body[0],
                alpha_body[1],
                alpha_body[2],  # Angular acceleration
            ]
        )
    ...
    def step(self):
            ...
            if isinstance(comp, Parachute):
                if comp.check_deployment(self.time, self.state.z, self.state.vz):
                    if not comp.deployed:
                        comp.deployed = True
                        print(
                            f"  🪂 {comp.name} deployed at t={self.time:.2f}s, "
        ...
        if v_mag > 1000.0:  # Supersonic limit check (Mach 3)
            print(
                f"  ⚠️  WARNING: Excessive velocity {v_mag:.1f} m/s detected at t={self.time:.2f}s"
            )
            # Cap velocity
            scale = 1000.0 / v_mag
            self.state.vx *= scale
    ...
    def log_data(self):
        ...
        cp = aero.cp_x
        static_margin = (
            (cp - cg) / self.rocket.body_diameter if self.rocket.body_diameter > 0 else 0
        )
    ...
    def run(self, max_time: float = 60.0, verbose: bool = True):
        ...
        while self.time < max_time:
            ...
            if verbose and self.time - last_print_time >= 1.0:
                v = math.sqrt(self.state.vx**2 + self.state.vy**2 + self.state.vz**2)
                print(
                    f"  t={self.time:.1f}s: h={self.state.z:.1f}m, v={v:.1f}m/s, "
    ...
    def plot_results(self, filename: Optional[str] = None):
        ...
        plt.suptitle(f"{self.rocket.name} - Robust 6DOF Simulation", fontsize=16, fontweight="bold")
...
def test_robust_simulator():
    ...
    # Run simulation
    sim = RobustSimulator(
        rocket=rocket, motor=motor, motor_position=0.70, launch_angle_deg=90.0, rail_length=1.5
    )
