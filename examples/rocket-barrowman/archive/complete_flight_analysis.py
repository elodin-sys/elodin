...
def visualize_complete_rocket(rocket, motor):
    ...
    def draw_component(comp, parent_x=0):
        ...
        if isinstance(comp, NoseCone):
            ...
            ax_main.text(
                abs_x + L / 2, R + 15, comp.name, ha="center", fontsize=8, fontweight="bold"
            )
            ...
            ax_main.add_patch(rect_bot)
            ax_main.text(
                abs_x + L / 2, R + 5, "Motor Mount", ha="center", fontsize=7, fontweight="bold"
            )
            ...
            R = comp.outer_radius * 1000
            rect = Rectangle(
                (abs_x, 0), L, R, facecolor=colors["lug"], edgecolor="black", linewidth=1, alpha=0.8
            )
    ...
    motor_rect_top = Rectangle(
        (motor_x, 0),
        motor_L,
        motor_R,
        facecolor=colors["motor"],
        edgecolor="black",
        linewidth=2,
        alpha=0.8,
    )
    motor_rect_bot = Rectangle(
        (motor_x, -motor_R),
        motor_L,
        motor_R,
        facecolor=colors["motor"],
    ...
    ax_main.axvline(
        cg, color="blue", linestyle="--", linewidth=2.5, label=f"CG = {cg:.1f} mm", alpha=0.8
    )
    ax_main.axvline(
        cp, color="red", linestyle="--", linewidth=2.5, label=f"CP = {cp:.1f} mm", alpha=0.8
    )
    ax_main.scatter(
        [cg], [0], color="blue", s=300, marker="o", zorder=10, edgecolor="darkblue", linewidth=3
    )
    ...
    ax_mass.bar(
        positions, masses, width=20, color="steelblue", edgecolor="navy", linewidth=1.5, alpha=0.8
    )
    ...
    table = ax_comp.table(
        cellText=table_data, cellLoc="left", loc="center", colWidths=[0.5, 0.25, 0.25]
    )
    ...
    plt.tight_layout()
    plt.savefig(
        "/home/kush-mahajan/elodin/examples/rocket-barrowman/complete_rocket_build.png",
        dpi=150,
        bbox_inches="tight",
    )
...
def plot_complete_telemetry(history, summary, rocket, motor):
    ...
    ax7.axhline(
        masses[0], color="darkred", linestyle="--", alpha=0.6, label=f"Ignition: {masses[0]:.1f} g"
    )
    ...
    summary_data = [
        ...
        [
            "Max Altitude",
            f"{summary['max_altitude']:.1f} m ({summary['max_altitude'] * 3.281:.0f} ft)",
        ],
        ...
        [
            "Burnout Altitude",
            f"{altitudes[burnout_idx]:.1f} m" if burnout_idx < len(altitudes) else "N/A",
        ],
        ["Coast Time", f"{summary['apogee_time'] - motor.burn_time:.2f} s"],
        ["Flight Time", f"{summary['flight_time']:.1f} s"],
        [
            "Rail Exit Vel.",
            f"{velocities[int(0.11 / 0.01)]:.1f} m/s" if len(velocities) > 11 else "N/A",
        ],
    ]

    table = ax9.table(cellText=summary_data, cellLoc="left", loc="center", colWidths=[0.5, 0.5])
    table.auto_set_font_size(False)
    ...
    plt.savefig(
        "/home/kush-mahajan/elodin/examples/rocket-barrowman/complete_flight_telemetry.png",
        dpi=150,
        bbox_inches="tight",
    )
...
if __name__ == "__main__":
    ...
    print("\nFlight Summary:")
    print(
        f"  Apogee:        {summary['max_altitude']:.1f} m ({summary['max_altitude'] * 3.281:.0f} ft)"
    )
