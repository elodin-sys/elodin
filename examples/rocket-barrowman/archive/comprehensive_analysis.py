...
def build_rocket():
    ...
    body = BodyTube(name="Body tube", length=0.30, outer_radius=0.025, thickness=0.002)
    ...
    # Parachute - 30cm
    chute = Parachute(name="Parachute", diameter=0.30, cd=0.75, material_density=50.0)
...
def print_first_order_analysis(rocket: Rocket, motor: Motor):
    ...
    print(
        f"Reference area:        {math.pi * (rocket.reference_diameter / 2) ** 2 * 10000:.2f} cm²"
    )
    ...
    print(
        f"At burnout (T+{motor.burn_time:.1f}s):  {mass_burnout:.2f} g  ({mass_burnout / 1000:.4f} kg)"
    )
...
def print_second_order_analysis(rocket: Rocket, motor: Motor):
    ...
    time_labels = [
        "Ignition (T+0)",
        f"Mid-burn (T+{motor.burn_time / 2:.1f}s)",
        f"Burnout (T+{motor.burn_time:.1f}s)",
        "Coast (T+7s)",
    ]
    ...
    for t, label in zip(times, time_labels):
        ...
        total_cg = (
            (comp_mass * comp_cg + motor.get_mass(t) * (motor_mount_pos + motor_cg_local))
            / (comp_mass + motor.get_mass(t))
            * 1000
        )
...
def print_third_order_analysis(rocket: Rocket, motor: Motor):
    ...
    print(
        f"Maximum altitude:      {summary['max_altitude']:.1f} m ({summary['max_altitude'] * 3.281:.0f} ft)"
    )
    print(
        f"Maximum velocity:      {summary['max_velocity']:.1f} m/s ({summary['max_velocity'] * 2.237:.1f} mph)"
    )
...
def visualize_rocket(rocket: Rocket, motor: Motor):
    ...
    for comp in rocket.children:
        ...
        if isinstance(comp, NoseCone):
            ...
            rect_top = Rectangle(
                (pos, 0), L, R, facecolor="lightblue", edgecolor="darkblue", linewidth=2
            )
            rect_bot = Rectangle(
                (pos, -R), L, R, facecolor="lightblue", edgecolor="darkblue", linewidth=2
            )
    ...
    motor_rect_top = Rectangle(
        (motor_x, 0), motor_L, motor_R, facecolor="gray", edgecolor="black", linewidth=2, alpha=0.7
    )
    ...
    cg_ign = (
        (comp_mass * comp_cg + motor_mass_ign * motor_cg_ign) / (comp_mass + motor_mass_ign) * 1000
    )

    cp = aero.calculate_cp(0.0) * 1000

    ax1.axvline(
        cg_ign, color="blue", linestyle="--", linewidth=2, label=f"CG (ignition) = {cg_ign:.1f} mm"
    )
    ax1.axvline(cp, color="red", linestyle="--", linewidth=2, label=f"CP = {cp:.1f} mm")
    ax1.scatter(
        [cg_ign], [0], color="blue", s=200, marker="o", zorder=10, edgecolor="darkblue", linewidth=2
    )
    ...
    plt.tight_layout()
    plt.savefig(
        "/home/kush-mahajan/elodin/examples/rocket-barrowman/rocket_analysis.png",
        dpi=150,
        bbox_inches="tight",
    )
...
def main():
    ...
    plt.tight_layout()
    plt.savefig(
        "/home/kush-mahajan/elodin/examples/rocket-barrowman/flight_trajectory.png", dpi=150
    )
