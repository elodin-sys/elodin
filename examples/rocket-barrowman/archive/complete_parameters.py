...
def calculate_all_parameters(rocket, motor):
    ...
    cd_fin = 0.0
    if fin_component:
        cd_fin = aero.calculator.fin_drag(
            fin_component, 0.3, ref_area, v, props["density"], props["viscosity"]
        )
    ...
    params["motor"] = {
        ...
        "thrust_to_weight_ignition": motor.average_thrust
        / (params["mass"]["ignition"]["total_mass_kg"] * 9.81),
        "thrust_to_weight_burnout": motor.average_thrust
        / (params["mass"]["burnout"]["total_mass_kg"] * 9.81),
    }
...
def print_all_parameters(params):
    ...
    g = params["geometry"]
    print(
        f"Total Length:          {g['total_length'] * 1000:.2f} mm  ({g['total_length'] * 39.37:.3f} in)"
    )
    print(
        f"Maximum Diameter:      {g['max_diameter'] * 1000:.2f} mm  ({g['max_diameter'] * 39.37:.3f} in)"
    )
    ...
    print("─" * 100)
    print(
        f"{'Condition':<20} {'Mass (g)':<15} {'CG (mm)':<15} {'Ixx=Iyy (kg·m²)':<20} {'Izz (kg·m²)':<15}"
    )
    print("─" * 100)
    for label in ["ignition", "mid_burn", "burnout", "coast"]:
        m = params["mass"][label]
        print(
            f"{label.capitalize():<20} {m['total_mass_g']:<15.3f} {m['cg_position_mm']:<15.1f} "
    ...
    print("─" * 100)
    print(
        f"{'Mach':<8} {'CN_α (/rad)':<12} {'CM_α':<12} {'CD':<10} {'CP (mm)':<12} {'CN_q':<12} {'CM_q':<12}"
    )
    print("─" * 100)
    for key in sorted(params["aero_coefficients"].keys()):
        a = params["aero_coefficients"][key]
        print(
            f"{a['mach_number']:<8.1f} {a['cn_alpha']:<12.4f} {a['cm_alpha']:<12.4f} "
    ...
    s = params["stability"]
    print(
        f"Static Margin (ignition):  {s['static_margin_ignition_cal']:.3f} cal  ({s['static_margin_ignition_percent']:.1f}%)"
    )
    print(
        f"Static Margin (burnout):   {s['static_margin_burnout_cal']:.3f} cal  ({s['static_margin_burnout_percent']:.1f}%)"
    )
    ...
    print(
        f"  Body Friction:       {d['cd_body_friction']:.4f}  ({d['cd_body_friction'] / d['cd_total'] * 100:.1f}%)"
    )
    ...
    print(
        f"Apogee:                {f['apogee_m']:.1f} m  ({f['apogee_ft']:.0f} ft)  @ {f['apogee_time_s']:.2f} s"
    )
    print(
        f"Max Velocity:          {f['max_velocity_ms']:.1f} m/s  ({f['max_velocity_fps']:.0f} ft/s, {f['max_velocity_mph']:.1f} mph)"
    )
    print(f"Max Mach:              {f['max_mach']:.3f}")
    print(
        f"Max Acceleration:      {f['max_acceleration_g']:.1f} g  @ {f['max_acceleration_time_s']:.3f} s"
    )
    print(
        f"Rail Exit Velocity:    {f['rail_exit_velocity_ms']:.1f} m/s  @ {f['rail_exit_time_s']:.3f} s"
    )
    ...
    print(
        f"Descent Rate:          {f['descent_rate_ms']:.1f} m/s  ({f['descent_rate_fps']:.1f} ft/s)"
    )
    print(f"Flight Time:           {f['flight_time_s']:.1f} s")
    print(
        f"Max Dynamic Pressure:  {f['max_dynamic_pressure_pa']:.0f} Pa  @ {f['max_q_altitude_m']:.1f} m, {f['max_q_time_s']:.2f} s"
    )
    ...
    print("─" * 100)
    print(
        f"{'Altitude':<15} {'Temp (°C)':<12} {'Pressure (Pa)':<18} {'Density (kg/m³)':<18} {'Sound (m/s)':<15}"
    )
    print("─" * 100)
    for key in sorted(
        params["atmosphere"].keys(), key=lambda x: params["atmosphere"][x]["altitude_m"]
    ):
        a = params["atmosphere"][key]
        print(
            f"{a['altitude_m']:<15.0f} {a['temperature_c']:<12.1f} {a['pressure_pa']:<18.0f} "
...
if __name__ == "__main__":
    ...
    print(
        f"   Total parameters: {sum(len(v) if isinstance(v, dict) else 1 for v in params.values())}"
    )
