...
class StressAnalyzer:
    ...
    def body_tube_stress(
        ...
        bending_stress = bending_moment * outer_radius / I if I > 0 else 0
...
class StructuralAnalyzer:
    ...
    def analyze(self, flight_history: List) -> StructuralAnalysisResult:
        ...
        if fin_data:
            ...
            flutter_result.safety_margin = flutter_result.flutter_velocity / max_velocity if max_velocity > 0 else 10
        ...
        for snapshot in flight_history:
            if snapshot.dynamic_pressure > max_q:
                ...
                acceleration = np.linalg.norm(snapshot.acceleration) if hasattr(snapshot, 'acceleration') else 50
                thrust = self.motor.thrust(snapshot.time) if snapshot.time < self.motor.burn_time else 0
        ...
        if max_loads is None:
            max_loads = LoadsResult(
                time=0, altitude=0, velocity=0, mach=0, dynamic_pressure=0,
                axial_load=0, shear_load=0, bending_moment=0, torsion_moment=0,
                axial_acceleration=0, lateral_acceleration=0,
            )
        ...
        critical_component = min(component_stresses, key=lambda s: s.yield_margin).component_name if component_stresses else "None"
