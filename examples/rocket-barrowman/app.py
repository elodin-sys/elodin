...
from openrocket_components import (
    Rocket, NoseCone, BodyTube, TrapezoidFinSet, Transition,
    InnerTube, CenteringRing, MassComponent, Parachute, MATERIALS
)
...
if len(st.session_state.motor_database) == 0:
    try:
        ...
        if motor_db and len(motor_db) >= 50:
            ...
            openai_key = os.getenv('OPENAI_API_KEY') or st.session_state.get('openai_api_key')
            st.session_state.ai_designer = RocketDesigner(motor_db, openai_api_key=openai_key)
            st.warning(f"⚠️ Only {len(motor_db)} motors in database. Download full database from sidebar for better motor selection.")
    except Exception:
...
def build_custom_rocket(config: Dict[str, Any]) -> Rocket:
    ...
    if config.get('has_nose', True):
        ...
        nose.material = MATERIALS.get(config.get('nose_material', 'Fiberglass'), MATERIALS['Fiberglass'])
        nose.position.x = 0.0
    ...
    body.material = MATERIALS.get(config.get('body_material', 'Fiberglass'), MATERIALS['Fiberglass'])
    ...
    if config.get('has_fins', True):
        ...
        fins.material = MATERIALS.get(config.get('fin_material', 'Fiberglass'), MATERIALS['Fiberglass'])
    ...
    if config.get('has_motor_mount', True):
        ...
        motor_mount.material = MATERIALS.get(config.get('motor_mount_material', 'Fiberglass'), MATERIALS['Fiberglass'])
        body_length = config.get('body_length', 1.5)
        nose_length = config.get('nose_length', 0.5) if config.get('has_nose', True) else 0.0
...
def build_custom_motor(config: Dict[str, Any]) -> ORMotor:
    ...
    if 'thrust_curve' in config and config['thrust_curve']:
        ...
        burn_time = config.get('burn_time', max([t for t, _ in thrust_curve]) if thrust_curve else 3.9)
        total_impulse = config.get('total_impulse', sum((thrust_curve[i][1] + thrust_curve[i+1][1])/2 * 
                                                         (thrust_curve[i+1][0] - thrust_curve[i][0]) 
                                                         for i in range(len(thrust_curve)-1)) if len(thrust_curve) > 1 else 0)
    else:
...
def visualize_results(result: FlightResult):
    ...
    with tab1:
        ...
        with col1:
            # Altitude vs Time
            fig = go.Figure()
            fig.add_trace(go.Scatter(x=times, y=altitudes, mode='lines', name='Altitude', line=dict(color='blue')))
            fig.update_layout(
        ...
        with col2:
            # Velocity vs Time
            fig = go.Figure()
            fig.add_trace(go.Scatter(x=times, y=velocities, mode='lines', name='Velocity', line=dict(color='red')))
            fig.update_layout(
        ...
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=downrange, y=altitudes, mode='lines', name='Trajectory', line=dict(color='green')))
        fig.update_layout(
    ...
    with tab2:
        ...
        with col2:
            # Mach number
            fig = go.Figure()
            fig.add_trace(go.Scatter(x=times, y=machs, mode='lines', name='Mach', line=dict(color='purple')))
            fig.update_layout(
    ...
    with tab3:
        ...
        with col1:
            # Angle of Attack
            fig = go.Figure()
            fig.add_trace(go.Scatter(x=times, y=aoas, mode='lines', name='AoA', line=dict(color='orange')))
            fig.update_layout(
        ...
        with col2:
            # Dynamic Pressure
            fig = go.Figure()
            fig.add_trace(go.Scatter(x=times, y=dynamic_pressures, mode='lines', name='Dynamic Pressure', line=dict(color='cyan')))
            fig.update_layout(
...
def launch_elodin_editor(result: FlightResult, solver: FlightSolver):
    ...
    try:
        ...
        with st.spinner("Launching Elodin editor..."):
                    ...
                    if wrapper_script.exists():
                        ...
                        for term_base, shell_prefix in terminals[:3]:  # Try first 3 terminals
                            try:
                                ...
                                if which_result.returncode == 0:
                                    if shell_prefix == 'bash -c':
                                        full_cmd = term_base + ['bash', '-c', cmd_str + '; read -p "Press Enter to close..."']
                                    else:
                                        full_cmd = term_base + [cmd_str]
            ...
            if process:
                if process.poll() is None:  # Process is still running
                    ...
                    st.info(f"💡 Try running manually: `cd {script_dir} && python3 main.py --visualize`")
            else:
        ...
        st.info(f"💡 Result file exists: {result_file.exists() if 'result_file' in locals() else 'N/A'}")
        st.info(f"💡 Solver file exists: {solver_file.exists() if 'solver_file' in locals() else 'N/A'}")
...
def main():
    ...
    with st.sidebar:
                        ...
                        try:
                            ...
                            with st.expander("📊 Rocket Design Summary", expanded=True):
                                ...
                                with col1:
                                    ...
                                    st.write(f"• Total Length: {config['nose_length'] + config['body_length']:.2f} m")
                                    st.write(f"• Body Diameter: {config['body_radius'] * 2 * 1000:.1f} mm")
                                    ...
                                    if dry_mass_display == 0 or dry_mass_display is None:
                                        ...
                                        if mass_breakdown and isinstance(mass_breakdown, dict):
                                            dry_mass_display = mass_breakdown.get('total_dry_mass', 0)
                                    ...
                                    if dry_mass_display == 0 or dry_mass_display is None:
                                        try:
                                            ...
                                            if st.session_state.get('ai_designer'):
                                                designer = st.session_state.ai_designer
                                                req = designer.parse_requirements(ai_input) if 'ai_input' in locals() else None
                                                if req:
                                                    mass_breakdown = designer._calculate_comprehensive_mass(config, req)
                                                    dry_mass_display = mass_breakdown.get('total_dry_mass', 0)
                                    ...
                                    st.write(f"• Total Mass: {dry_mass_display + motor_mass:.2f} kg")
                                
                                with col2:
                                    st.markdown("**Performance Targets:**")
                                    if req.target_altitude_m:
                                        st.write(f"• Target Altitude: {req.target_altitude_m:.1f} m ({req.target_altitude_m * 3.28084:.0f} ft)")
                                    ...
                                    if motor_config:
                                        ...
                                        st.write(f"• Total Impulse: {motor_config['total_impulse']:.0f} N·s")
                                        st.write(f"• Avg Thrust: {motor_config['avg_thrust']:.0f} N")
                            ...
                            with st.expander("🔧 Detailed Configuration"):
                                st.json({
                                    ...
                                    "Recovery": {
                                        "Main Chute": f"{config.get('main_chute_diameter', 0):.2f} m" if config.get('has_main_chute') else "None",
                                        "Drogue": f"{config.get('drogue_diameter', 0):.2f} m" if config.get('has_drogue') else "None",
                                        "Main Deployment": config.get('main_deployment_event', 'ALTITUDE'),
                            ...
                            if config.get('space_claims'):
                                with st.expander("📐 Space Claims & Component Positions", expanded=False):
                                    ...
                                    for comp_name, comp_info in positions.items():
                                        if comp_info.get('start') is not None:
                                            position_data.append({
                                                ...
                                                'Diameter (mm)': f"{comp_info.get('diameter', 0)*1000:.1f}" if comp_info.get('diameter') else "N/A",
                                                'Clearance': comp_info.get('clearance_required', 'N/A')
                                            })
                                    ...
                                    if positions.get('main_parachute'):
                                        ...
                                        st.write(f"  - Location: {main_chute.get('location', 'N/A')}")
                                        st.write(f"  - Diameter: {main_chute.get('diameter', 0)*1000:.0f} mm")
                                        st.write(f"  - Deployment: {main_chute.get('deployment_event', 'N/A')} at {main_chute.get('deployment_altitude', 0):.0f} m")
                                        st.write(f"  - Clearance: {main_chute.get('clearance_required', 'N/A')}")
                                    
                                    if positions.get('drogue_parachute') and positions['drogue_parachute'].get('diameter', 0) > 0:
                                        drogue_chute = positions['drogue_parachute']
                                        st.write(f"**Drogue Parachute:**")
                                        st.write(f"  - Location: {drogue_chute.get('location', 'N/A')}")
                                        st.write(f"  - Diameter: {drogue_chute.get('diameter', 0)*1000:.0f} mm")
                                        st.write(f"  - Deployment: {drogue_chute.get('deployment_event', 'N/A')} at {drogue_chute.get('deployment_altitude', 0):.0f} m")
                                        st.write(f"  - Clearance: {drogue_chute.get('clearance_required', 'N/A')}")
                                    
                                    st.markdown("### Clearances & Fit Requirements")
                                    col1, col2 = st.columns(2)
                                    with col1:
                                        st.markdown("**Clearances:**")
                                        for clearance, desc in space_claims.get('clearances', {}).items():
                                            st.write(f"  • {clearance.replace('_', ' ').title()}: {desc}")
                                    with col2:
                                        st.markdown("**Fit Requirements:**")
                                        for fit, desc in space_claims.get('fit_requirements', {}).items():
                                    ...
                                    for vol_name, vol_value in volumes.items():
                                        st.write(f"  • {vol_name.replace('_', ' ').title()}: {vol_value}")
                            ...
                            if config.get('bom'):
                                with st.expander("📋 Bill of Materials (BOM)", expanded=False):
                                    ...
                                    st.dataframe(bom_df[['part_number', 'description', 'quantity', 'material', 'dimensions', 'mass_kg', 'notes']], 
            ...
            with st.expander("🔧 Motor Database"):
                ...
                if st.session_state.motor_database:
                    ...
                    if len(motors_with_impulse) < len(st.session_state.motor_database) * 0.5:
                        st.warning(f"⚠️ {len(st.session_state.motor_database)} motors loaded, but {len(st.session_state.motor_database) - len(motors_with_impulse)} have invalid impulse data. Please re-download motors to fix this.")
                    else:
                        st.success(f"✅ {len(st.session_state.motor_database)} motors loaded in database ({len(motors_with_impulse)} with valid impulse data)")
                else:
                ...
                with col2:
                    if st.button("🌐 Download/Update Motors", type="primary", use_container_width=True):
                        with st.spinner("Downloading full motor database from ThrustCurve.org API (this may take several minutes)..."):
                            try:
                                scraper = ThrustCurveScraper()
                                ...
                                motors = scraper.scrape_motor_list(max_motors=10000)  # Get all motors
                                if motors:
                                    ...
                                    if st.session_state.ai_designer:
                                        ...
                                        openai_key = os.getenv('OPENAI_API_KEY') or st.session_state.get('openai_api_key')
                                        st.session_state.ai_designer = RocketDesigner(motors, openai_api_key=openai_key)
                                    
                                    # Count motors with valid impulse
                                    valid_motors = [m for m in motors if m.total_impulse > 0]
                                    st.success(f"✅ Downloaded {len(motors)} motors from API ({len(valid_motors)} with valid impulse data)")
                ...
                if st.session_state.motor_database:
                    ...
                    if motor_classes:
                        ...
                        classes_str = ", ".join([f"{cls}: {count}" for cls, count in sorted(motor_classes.items())])
        ...
        if rocket_type != "AI Builder":
            ...
            if has_nose:
                nose_length = st.number_input("Nose Length (m)", min_value=0.1, max_value=2.0, value=0.55829, step=0.01)
                nose_thickness = st.number_input("Nose Thickness (m)", min_value=0.001, max_value=0.01, value=0.003, step=0.001)
            ...
            # Body tube
            body_length = st.number_input("Body Length (m)", min_value=0.1, max_value=5.0, value=1.5, step=0.1)
            body_radius = st.number_input("Body Radius (m)", min_value=0.01, max_value=0.5, value=0.0635, step=0.001)
            body_thickness = st.number_input("Body Thickness (m)", min_value=0.001, max_value=0.01, value=0.003, step=0.001)
            ...
            if has_fins:
                ...
                fin_root_chord = st.number_input("Fin Root Chord (m)", min_value=0.01, max_value=0.5, value=0.12, step=0.01)
                fin_tip_chord = st.number_input("Fin Tip Chord (m)", min_value=0.01, max_value=0.5, value=0.06, step=0.01)
                fin_span = st.number_input("Fin Span (m)", min_value=0.01, max_value=0.5, value=0.11, step=0.01)
                fin_sweep = st.number_input("Fin Sweep (m)", min_value=0.0, max_value=0.5, value=0.06, step=0.01)
                fin_thickness = st.number_input("Fin Thickness (m)", min_value=0.001, max_value=0.01, value=0.005, step=0.001)
                fin_material = st.selectbox("Fin Material", list(MATERIALS.keys()), index=3)
            
            # Motor mount
            has_motor_mount = st.checkbox("Motor Mount", value=True)
            if has_motor_mount:
                motor_mount_length = st.number_input("Motor Mount Length (m)", min_value=0.1, max_value=2.0, value=0.5, step=0.1)
                motor_mount_radius = st.number_input("Motor Mount Radius (m)", min_value=0.01, max_value=0.1, value=0.041, step=0.001)
                motor_mount_thickness = st.number_input("Motor Mount Thickness (m)", min_value=0.001, max_value=0.01, value=0.003, step=0.001)
                motor_mount_material = st.selectbox("Motor Mount Material", list(MATERIALS.keys()), index=3)
            
            # Parachutes
            st.subheader("Parachutes")
            has_main_chute = st.checkbox("Main Parachute", value=True)
            if has_main_chute:
                main_chute_diameter = st.number_input("Main Chute Diameter (m)", min_value=0.1, max_value=10.0, value=2.91, step=0.1)
                main_chute_cd = st.number_input("Main Chute CD", min_value=0.1, max_value=2.0, value=1.5, step=0.1)
                main_deployment_event = st.selectbox("Main Deployment Event", ["APOGEE", "ALTITUDE", "TIME"], index=1)
                if main_deployment_event == "ALTITUDE":
                    main_deployment_altitude = st.number_input("Main Deployment Altitude (m)", min_value=0.0, max_value=10000.0, value=800.0, step=10.0)
                else:
                    main_deployment_altitude = 0.0
                main_deployment_delay = st.number_input("Main Deployment Delay (s)", min_value=0.0, max_value=10.0, value=1.5, step=0.1)
            
            has_drogue = st.checkbox("Drogue Parachute", value=True)
            if has_drogue:
                drogue_diameter = st.number_input("Drogue Diameter (m)", min_value=0.1, max_value=5.0, value=0.99, step=0.1)
                drogue_cd = st.number_input("Drogue CD", min_value=0.1, max_value=2.0, value=1.3, step=0.1)
                drogue_deployment_event = st.selectbox("Drogue Deployment Event", ["APOGEE", "ALTITUDE", "TIME"], index=0)
                if drogue_deployment_event == "ALTITUDE":
                    drogue_deployment_altitude = st.number_input("Drogue Deployment Altitude (m)", min_value=0.0, max_value=10000.0, value=1000.0, step=10.0)
                else:
                    drogue_deployment_altitude = 0.0
                drogue_deployment_delay = st.number_input("Drogue Deployment Delay (s)", min_value=0.0, max_value=10.0, value=1.5, step=0.1)
            ...
            if st.session_state.motor_database and len(st.session_state.motor_database) > 0:
                ...
                if motor_type == "Select from Database":
                    ...
                    if motors_by_class:
                        ...
                        motors_in_class = sorted(motors_by_class[selected_class], 
                                               key=lambda m: m.total_impulse)
                        ...
                        if st.session_state.rocket_config:
                            ...
                            st.session_state.rocket_config['motor_mount_radius'] = (motor_diameter / 2.0) + 0.005  # 5mm clearance
                            st.session_state.rocket_config['motor_mount_length'] = motor_length + 0.1  # 10cm extra
                            
                            st.success(f"✅ Motor mount automatically updated: "
                        ...
                        # Display motor info
                        st.info(f"**Selected:** {selected_motor.designation} | "
            ...
            if motor_type == "Custom Motor":
                ...
                motor_diameter = st.number_input("Motor Diameter (m)", min_value=0.01, max_value=0.5, value=0.075, step=0.001)
                motor_length = st.number_input("Motor Length (m)", min_value=0.1, max_value=2.0, value=0.64, step=0.01)
                motor_total_mass = st.number_input("Total Mass (kg)", min_value=0.1, max_value=50.0, value=4.771, step=0.1)
                motor_propellant_mass = st.number_input("Propellant Mass (kg)", min_value=0.1, max_value=50.0, value=2.956, step=0.1)
                max_thrust = st.number_input("Max Thrust (N)", min_value=100.0, max_value=10000.0, value=2200.0, step=100.0)
                avg_thrust = st.number_input("Avg Thrust (N)", min_value=100.0, max_value=10000.0, value=1545.0, step=100.0)
                burn_time = st.number_input("Burn Time (s)", min_value=0.1, max_value=30.0, value=3.9, step=0.1)
            ...
            elevation = st.number_input("Launch Site Elevation (m)", min_value=0.0, max_value=5000.0, value=1400.0, step=10.0)
            
            # Launch conditions
            st.subheader("Launch Conditions")
            rail_length = st.number_input("Rail Length (m)", min_value=0.5, max_value=20.0, value=5.2, step=0.1)
            inclination_deg = st.number_input("Launch Angle (deg from vertical)", min_value=0.0, max_value=90.0, value=5.0, step=1.0)
            heading_deg = st.number_input("Heading (deg, 0=North)", min_value=0.0, max_value=360.0, value=0.0, step=1.0)
            
            # Simulation parameters
            st.subheader("Simulation")
            max_time = st.number_input("Max Simulation Time (s)", min_value=10.0, max_value=1000.0, value=200.0, step=10.0)
            dt = st.number_input("Time Step (s)", min_value=0.001, max_value=0.1, value=0.01, step=0.001, format="%.3f")
        else:
    ...
    with col1:
        ...
        show_config = st.session_state.rocket_config and (rocket_type != "AI Builder" or not st.session_state.get('ai_design_just_generated', False))
        ...
        if show_config:
            with st.expander("📋 Current Rocket Configuration", expanded=False):
                ...
                with col_a:
                    st.markdown("**Dimensions:**")
                    st.write(f"• Length: {config.get('nose_length', 0) + config.get('body_length', 0):.2f} m")
                ...
                if st.session_state.motor_config:
                    ...
                    st.markdown("**Motor:**")
                    st.write(f"• {motor.get('designation', 'N/A')} ({motor.get('manufacturer', 'N/A')})")
