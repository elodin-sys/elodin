...
def create_nose_cone_mesh(
    ...
    for xi in x:
        ...
        if shape == "CONICAL":
            ...
            r = base_radius * math.sqrt((theta_vk - math.sin(theta_vk) * math.cos(theta_vk)) / math.pi)
        elif shape == "HAACK":
...
def create_fin_mesh(
    ...
    return (
        ...
        np.array(i_list), np.array(j_list), np.array(k_list)
    )
...
def render_rocket_3d(
    ...
    for fi in range(fin_count):
        ...
        x, y, z, i, j, k = create_fin_mesh(
            fin_start_x, fin_root, fin_tip, fin_span, fin_sweep,
            body_radius, angle, fin_thickness
        )
        fig.add_trace(go.Mesh3d(
            x=x, y=y, z=z, i=i, j=j, k=k,
            color=colors["fins"],
    ...
    if motor or show_internals:
        ...
        x, y, z, i, j, k = create_cylinder_mesh(motor_start, motor_length, motor_radius * 0.95, n_theta=24)
        fig.add_trace(go.Mesh3d(
            x=x, y=y, z=z, i=i, j=j, k=k,
            color=colors["motor"],
            opacity=0.9,
...
def render_rocket_profile(
    ...
    # Fin (upper)
    fig.add_trace(go.Scatter(
        x=[fin_start, fin_start + fin_root, fin_start + fin_sweep + fin_tip, fin_start + fin_sweep, fin_start],
        y=[body_radius, body_radius, body_radius + fin_span, body_radius + fin_span, body_radius],
        fill="toself",
    ...
    motor_start = total_length - motor_length - 0.02
    fig.add_trace(go.Scatter(
        x=[motor_start, motor_start + motor_length, motor_start + motor_length, motor_start, motor_start],
