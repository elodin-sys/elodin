...
from core import (  # noqa: E402
    Environment,
    Motor,
    Rocket as RocketModel,
    FlightSolver,
    FlightResult,
)
from core.components.openrocket_components import (  # noqa: E402
    Rocket,
    NoseCone,
    BodyTube,
    TrapezoidFinSet,
...
from core.components.openrocket_motor import Motor as ORMotor  # noqa: E402
from core.data.motor_scraper import ThrustCurveScraper  # noqa: E402
from core.builders.calisto_builder import build_calisto  # noqa: E402
from ui.rocket_visualizer import visualize_rocket_3d, visualize_rocket_2d_side_view  # noqa: E402
from optimization.ai_rocket_builder import RocketDesigner  # noqa: E402
from optimization.smart_optimizer import SmartOptimizer  # noqa: E402
from analysis import FlightAnalyzer, compute_flight_phases  # noqa: E402
    ...
    with tab8:  # Environment
        ...
        if solver and solver.environment:
            ...
            if len(history) > 0:
                ...
                fig.update_layout(
                    **create_chart_layout(
                        "Wind Speed During Flight", "Time (s)", "Wind Speed (m/s)"
                    )
                )
                fig.update_layout(height=300)
                st.plotly_chart(fig, width="stretch")
    ...
    with tab9:  # Structural Analysis
        ...
        if solver:
            try:
                ...
                fin_mat_props = STRUCTURAL_MATERIALS.get(
                    fin_material, STRUCTURAL_MATERIALS["Fiberglass"]
                )
                body_mat_props = STRUCTURAL_MATERIALS.get(
                    body_material, STRUCTURAL_MATERIALS["Fiberglass"]
                )
                ...
                flutter_margin = (
                    flutter_vel / max_flight_vel if max_flight_vel > 0 else float("inf")
                )
                divergence_margin = (
                    divergence_vel / max_flight_vel if max_flight_vel > 0 else float("inf")
                )
                ...
                with col1:
                    flutter_color = (
                        ...
                        else (COLORS["warning"] if flutter_margin > 1.0 else COLORS["danger"])
                    )
                    st.markdown(
                        f"""
                ...
                with col2:
                    diverge_color = (
                        ...
                        else (COLORS["warning"] if divergence_margin > 1.0 else COLORS["danger"])
                    )
                    st.markdown(
                        f"""
                ...
                if flutter_margin < 1.0:
                    st.error(
                        "⚠️ **FLUTTER RISK**: Fin flutter velocity exceeded during flight! Consider thicker fins or stiffer material."
                    )
                elif flutter_margin < 1.25:
                    st.warning(
                        "⚠️ Flutter margin is low. Consider increasing fin thickness or using stiffer material."
                    )
                else:
                ...
                aspect_ratio = (
                    2 * fin_span / (fin_root + fin_tip) if (fin_root + fin_tip) > 0 else 1.0
                )
                st.markdown(f"""
                | Parameter | Value |
                |-----------|-------|
                ...
                # Get mass properties
                rocket_mass = (
                    solver.mass_model.total_mass(0) if hasattr(solver, "mass_model") else 10.0
                )
                ...
                motor_thrust = (
                    solver.motor.max_thrust if hasattr(solver.motor, "max_thrust") else 2000.0
                )
                ...
                # Axial stress
                axial_stress = (
                    abs(loads_result.axial_load) / cross_section_area
                ...
                bending_stress = (
                    loads_result.bending_moment * body_radius / moment_of_inertia
                    if moment_of_inertia > 0
                    else 0
                )

                # Combined stress (von Mises approximation)
                body_stress = math.sqrt(
                    axial_stress**2 + bending_stress**2 + axial_stress * bending_stress
                )
                ...
                with col1:
                    body_color = (
                        COLORS["success"]
                        if body_margin > 2.0
                        else (COLORS["warning"] if body_margin > 1.5 else COLORS["danger"])
                    )
                    st.markdown(
                        f"""
                ...
                with col2:
                    fin_color = (
                        COLORS["success"]
                        if fin_margin > 2.0
                        else (COLORS["warning"] if fin_margin > 1.5 else COLORS["danger"])
                    )
                    st.markdown(
                        f"""
                ...
                if flutter_margin < 1.5:
                    recommendations.append(
                        f"• **Fin Flutter**: Increase fin thickness from {fin_thickness * 1000:.1f}mm to {fin_thickness * 1000 * 1.5:.1f}mm or use stiffer material (Carbon Fiber recommended)"
                    )

                if body_margin < 2.0:
                    recommendations.append(
                        f"• **Body Stress**: Increase wall thickness from {body_thickness * 1000:.1f}mm or upgrade material"
                    )

                if fin_margin < 2.0:
                    recommendations.append(
                        "• **Fin Stress**: Consider thicker fins or fiberglass reinforcement at root"
                    )

                if divergence_margin < 2.0:
                    recommendations.append(
                        "• **Divergence**: Reduce fin span or increase torsional stiffness"
                    )
                ...
                if rocket_config.get("num_rail_buttons", 0) > 0:
                    ...
                    btn_dims = {
                        "mini": {"d": 0.008, "h": 0.010},
                        "standard": {"d": 0.010, "h": 0.015},
                        "1010": {"d": 0.010, "h": 0.010},
                        "1515": {"d": 0.015, "h": 0.015},
                    }.get(btn_size, {"d": 0.010, "h": 0.015})
                ...
                if rocket_config.get("has_camera", False):
                    ...
                    cd_cam = {"blister": 0.45, "streamlined": 0.12, "flush": 0.03}.get(
                        cam_type, 0.45
                    )
                ...
                if prot_info:
                    ...
                    st.dataframe(df_prot, width="stretch", hide_index=True)
                    ...
                    st.caption(
                        f"Total protuberance frontal area: {total_prot_frontal * 1e4:.3f} cm² ({prot_drag_ratio:.1f}% of reference area)"
                    )
                else:
                    st.info(
                        "No external protuberances configured. Add rail buttons, launch lugs, or camera in the Hardware tab."
                    )
...
def launch_elodin_editor(result: FlightResult, solver: FlightSolver):
    ...
    try:
        ...
        script_dir = Path(__file__).parent.resolve()  # ui/ directory
        rocket_barrowman_dir = script_dir.parent.resolve()  # examples/rocket-barrowman/
...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        if current_rocket_type == "AI Builder":
            ...
            if st.button("🔍 Find Cheapest Design", type="primary", width="stretch"):
                if ai_input:
        ...
        with st.expander("🔧 Motor Database", expanded=False):
            ...
            if st.button("🌐 Download Motors", type="primary", width="stretch"):
            ...
            if st.button("📥 Load from Cache", width="stretch"):
        ...
        if current_rocket_type == "Custom Rocket":
            ...
            config_tabs = st.tabs(
                ["🔺 Nose", "📦 Body", "🔗 Structure", "🔱 Fins", "🪂 Recovery", "📍 Hardware"]
            )
                ...
                if has_nose:
                    ...
                    with col1:
                        nose_length = st.number_input(
                            "Length", 0.1, 2.0, 0.558, 0.01, key="nose_len", format="%.2f"
                        )
                    with col2:
                        nose_shape = st.selectbox(
                            "Shape", ["VON_KARMAN", "OGIVE", "CONICAL", "HAACK"], key="nose_shape"
                        )
            ...
            with config_tabs[1]:
                ...
                with col2:
                    body_radius = st.number_input(
                        "Radius (m)", 0.01, 0.5, 0.0635, 0.005, key="body_rad"
                    )
            ...
            with config_tabs[2]:
                ...
                with col1:
                    motor_mount_length = st.number_input(
                        "Mount Length (m)", 0.1, 1.0, 0.5, 0.05, key="mm_length"
                    )
                    motor_mount_material = st.selectbox(
                        "Mount Material", list(MATERIALS.keys()), index=3, key="mm_mat"
                    )
                with col2:
                    # Motor mount diameter should match motor (set automatically from motor config)
                    motor_mount_od = (
                        st.number_input("Mount OD (mm)", 20.0, 100.0, 75.0, 1.0, key="mm_od") / 1000
                    )
                    motor_mount_wall = (
                        st.number_input("Wall (mm)", 1.0, 5.0, 2.0, 0.5, key="mm_wall") / 1000
                    )

                # Centering rings
                st.markdown("**Centering Rings**")
                num_centering_rings = st.number_input(
                    "Centering Rings",
                    1,
                    5,
                    2,
                ...
                if has_fins:
                    ...
                    with col1:
                        ...
                        fin_span = st.number_input(
                            "Span (m)", 0.01, 0.5, 0.11, 0.01, key="fin_span"
                        )
                        fin_thickness = (
                            st.number_input("Thickness (mm)", 1.0, 15.0, 5.0, 0.5, key="fin_thick")
                            / 1000
                        )  # Convert to m
                    with col2:
                        ...
                        fin_sweep = st.number_input(
                            "Sweep Length (m)", 0.0, 0.5, 0.06, 0.01, key="fin_sweep"
                        )
                        fin_material = st.selectbox(
                            "Material", list(MATERIALS.keys()), index=3, key="fin_mat"
                        )  # Fiberglass default
                    ...
                    with col2:
                        if airfoil_type == "NACA 4-digit":
                            ...
                            le_radius = st.number_input(
                                ...
                                5.0,
                                key="le_radius",
                                help="Leading edge radius as % of thickness",
                            )
                            naca_code = "0012"  # default
                            wedge_angle = 10.0  # default
                else:
                    fin_count, fin_root_chord, fin_span, fin_tip_chord, fin_sweep = (
                        4,
                        0.12,
                        0.11,
                        0.06,
            ...
            with config_tabs[5]:
                ...
                col1, col2 = st.columns(2)
                with col1:
                    num_rail_buttons = st.number_input(
                        "Rail Buttons", 0, 4, 2, 1, key="num_rail_btns"
                    )
                with col2:
                    rail_btn_size = st.selectbox(
                        "Size", ["mini", "standard", "1010", "1515"], index=1, key="rail_size"
                    )

                if num_rail_buttons > 0:
                    col1, col2 = st.columns(2)
                    with col1:
                        rail_btn_pos1 = st.number_input(
                            "Btn 1 pos (m)", 0.1, 5.0, 0.3, 0.05, key="rail_pos1"
                        )
                    with col2:
                        if num_rail_buttons > 1:
                            rail_btn_pos2 = st.number_input(
                                "Btn 2 pos (m)", 0.1, 5.0, 1.5, 0.05, key="rail_pos2"
                            )
                        else:
                            rail_btn_pos2 = 0.0
                else:
                ...
                with col2:
                    lug_rod_size = st.selectbox(
                        "Rod", ["1/8", "3/16", "1/4"], index=1, key="lug_rod"
                    )

                if num_launch_lugs > 0:
                    col1, col2 = st.columns(2)
                    with col1:
                        lug_length = st.number_input(
                            "Length (m)", 0.01, 0.1, 0.03, 0.005, key="lug_len"
                        )
                        lug_pos1 = st.number_input(
                            "Lug 1 pos (m)", 0.1, 5.0, 0.3, 0.05, key="lug_pos1"
                        )
                    with col2:
                        if num_launch_lugs > 1:
                            lug_pos2 = st.number_input(
                                "Lug 2 pos (m)", 0.1, 5.0, 1.5, 0.05, key="lug_pos2"
                            )
                        else:
                            lug_pos2 = 0.0
                else:
                ...
                if has_camera:
                    ...
                    with col1:
                        camera_type = st.selectbox(
                            "Type", ["blister", "streamlined", "flush"], key="cam_type"
                        )
                        camera_pos = st.number_input(
                            "Position (m)", 0.1, 5.0, 0.5, 0.05, key="cam_pos"
                        )
                    with col2:
                        camera_length = st.number_input(
                            "Length (m)", 0.02, 0.2, 0.05, 0.01, key="cam_len"
                        )
                        camera_height = st.number_input(
                            "Height (m)", 0.005, 0.05, 0.015, 0.005, key="cam_ht"
                        )
                else:
                    camera_type, camera_pos, camera_length, camera_height = (
                        "blister",
                        0.5,
                        0.05,
                        0.015,
            ...
            rocket_config = {
                ...
                if (has_fins and airfoil_type == "Double Wedge")
                else 10.0,
        ...
        with st.expander("Launch Conditions", expanded=False):
            ...
            with col1:
                ...
                inclination_deg = st.number_input(
                    "Inclination (°)", 0.0, 90.0, 5.0, 1.0, key="incl"
                )
...
def main():
    ...
    with rocket_selector_col2:
        if st.button("🔄 Clear Results", width="stretch", help="Clear all simulation data"):
            ...
            with viz_tab1:
                if TRIMESH_AVAILABLE:
                    ...
                    try:
                        ...
                        with export_cols[0]:
                            from ui.mesh_renderer import export_stl
                        ...
                        with export_cols[1]:
                            from ui.mesh_renderer import export_gltf
                        ...
                        with export_cols[2]:
                            from ui.mesh_renderer import export_obj
    ...
    with col_actions:
        ...
        if st.button("🚀 LAUNCH", type="primary", width="stretch"):
            with st.spinner("Running simulation..."):
                try:
                    ...
                    if use_weather_data and launch_date and launch_time:
                        ...
                        from core.environment.weather_fetcher import (
        ...
        if st.session_state.simulation_result is not None:
            if st.button("🎮 Elodin Editor", width="stretch"):
