...
def create_environment_from_coordinates(
    ...
    if use_weather_data and REQUESTS_AVAILABLE:
        try:
            ...
            if weather_data:
                print(
                    f"✓ Fetched weather data from Open-Meteo for {latitude:.2f}°N, {longitude:.2f}°E"
                )
        except Exception as e:
    ...
    if weather_data:
        try:
            ...
            if wind_speed is not None and wind_direction is not None:
                ...
                profile_points = [
                    ProfilePoint(
                        altitude=0.0, speed=wind_speed, direction_rad=direction_rad, vertical=0.0
                    ),
                    ProfilePoint(
                        altitude=1000.0,
                        speed=wind_speed * 1.2,
                        direction_rad=direction_rad,
                        vertical=0.0,
                    ),
                    ProfilePoint(
                        altitude=5000.0,
                        speed=wind_speed * 1.5,
                        direction_rad=direction_rad,
                        vertical=0.0,
                    ),
                    ProfilePoint(
