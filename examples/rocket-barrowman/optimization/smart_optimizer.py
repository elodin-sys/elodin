...
class Requirements:
    ...
    total_mass_kg: Optional[float] = (
        None  # Total rocket mass including motor (if "weighs X" specified)
    )
...
class SmartOptimizer:
    ...
    def _estimate_dry_mass(
        ...
        nose_volume = (
            0.5 * math.pi * (nose_outer_radius**2 - nose_inner_radius**2) * nose_length * 0.7
        )  # ~70% of length is shell
    ...
    def _build_and_simulate(
        ...
        # Use heuristics to adjust structure if needed to meet total mass
        ...
        if requirements.total_mass_kg is not None:
            ...
            # If structure + motor exceeds total, use heuristics to reduce structure
            if payload_mass < 0:
                # Heuristic: try reducing body length first (less impact on performance)
                min_functional_length = motor.length + 0.5
                if body_length > min_functional_length:
                    body_length = min_functional_length
                    structure_mass = self._estimate_dry_mass(
                        tube, body_length, nose_length, motor, 0.0, requirements.recovery_type
                    )
                    payload_mass = requirements.total_mass_kg - structure_mass - motor.total_mass

                # If still too heavy, use thinner walls (heuristic: 0.002 instead of 0.003)
                if payload_mass < 0:
                    use_thinner_walls = True
                    # Approximate mass reduction from thinner walls (~10% reduction)
                    structure_mass = structure_mass * 0.9
                    payload_mass = requirements.total_mass_kg - structure_mass - motor.total_mass

                    # If still can't fit, skip this design
                    if payload_mass < 0:
                        return (0.0, 0.0, {}, {})
        ...
        body = BodyTube(
            name="Body", length=body_length, outer_radius=body_radius, thickness=wall_thickness
        )
    ...
    def optimize(
        ...
        if requirements.total_mass_kg is not None:
            log.append(
                f"Total mass: {requirements.total_mass_kg:.1f}kg (payload will be calculated)"
            )
        else:
        ...
        max_impulse_needed = max_alt / (
            altitude_per_impulse * 0.8
        ...
        if estimated_total_mass_kg > 30:
            ...
            min_impulse_filter = max(
                min_impulse_needed * 0.85, 5200
            )  # Exclude L-class (max 5120 N·s)
            viable_motors = [
                m
        ...
        if len(viable_motors) > 15:
            ...
            if estimated_total_mass_kg > 30:
                ...
                if (
                    upper_75_idx < len(viable_motors)
                    and viable_motors[upper_75_idx] not in phase1_motors
                ):
        ...
        if phase2_motors and best:
            ...
            for motor in phase2_motors[: max(15, max_iterations - iteration)]:
                ...
                if min_tube_for_payload and min_tube_for_payload["id"] > tube_for_motor["id"]:
                    # Validate payload tube also fits the motor
                    if (
                        min_tube_for_payload["od"] >= motor.diameter
                        and min_tube_for_payload["id"] >= motor.diameter + 0.001
                    ):
                        tube = min_tube_for_payload
                    else:
                        tube = tube_for_motor
