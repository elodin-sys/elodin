...
class RocketDesigner:
    ...
    SPELLING_VARIATIONS = {
        'altitude': ['altitude', 'alt', 'height', 'hight', 'hieght'],
        'payload': ['payload', 'paylod', 'pay load', 'load'],
        'parachute': ['parachute', 'parashute', 'chute', 'paracute'],
        'recover': ['recover', 'recovery', 'rrecover', 'recovr', 'land'],
        'diameter': ['diameter', 'diamter', 'width', 'size'],
        'length': ['length', 'lenght', 'long', 'tall'],
    ...
    def __init__(self, motor_database: Optional[List['MotorData']] = None, openai_api_key: Optional[str] = None):
        ...
        if self.use_openai:
            ...
            print("Using regex-based parsing (install openai and set OPENAI_API_KEY for better results)")
...
IMPORTANT:
- Handle misspellings (e.g., "rrecover" -> recovery, "hight" -> altitude)
...
- Return only valid JSON, no other text."""
            ...
            response = self.openai_client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": "You are an expert rocket design assistant. Extract ALL requirements from natural language, handle misspellings gracefully, and return comprehensive structured JSON. Be thorough and catch every detail."},
    ...
    def _parse_with_regex(self, text: str) -> RocketRequirements:
        ...
        altitude_patterns = [
            ...
            (r'(\d+\.?\d*)\s*ft\b', 0.3048),  # feet (word boundary to avoid matching "ft" in other words)
            (r'(\d+\.?\d*)\s*feet', 0.3048),
        ...
        for pattern, multiplier in altitude_patterns:
            ...
            if match:
                ...
                if 'ft' in pattern or 'feet' in pattern or 'thousand' in pattern or 'k\s*ft' in pattern:
        ...
        total_mass_patterns = [
            (r'(\d+\.?\d*)\s*(?:lb|lbs|pound|pounds)\s+rocket', 0.453592),  # "50 lb rocket" - HIGHEST PRIORITY
            (r'rocket\s+(?:that\s+)?(?:weighs?|mass|weight)\s+(\d+\.?\d*)', 0.453592),  # "rocket weighs 50"
            (r'(\d+\.?\d*)\s*(?:lb|lbs|pound|pounds)\s+(?:rocket|total)', 0.453592),  # "50 lb total"
            ...
            for iteration in range(max_iterations):
                ...
                if iteration == 0:
                    ...
                    required_impulse = self._estimate_required_impulse(
                        target_altitude,
                        requirements.payload_mass_kg or 0.0,
                        rough_diameter
                    )
                else:
                    # Refine based on previous iteration's error
                    if simulated_altitude > 0:
                        altitude_error_ratio = (simulated_altitude - target_altitude) / target_altitude
                        # If too high, reduce impulse; if too low, increase
                        impulse_adjustment = 1.0 - altitude_error_ratio * 0.3  # Damping factor
                        required_impulse *= max(0.5, min(2.0, impulse_adjustment))  # Limit adjustment
                ...
                if requirements.diameter_constraint_m:
                    ...
                    standard_radii = [0.019, 0.027, 0.0375, 0.049, 0.0635, 0.076, 0.10, 0.127, 0.15, 0.20]
                    ...
                    payload_size_estimate = max(config.get('payload_width', 0.1), config.get('payload_height', 0.1))
    ...
    def _estimate_required_impulse(self, target_altitude_m: float, payload_mass_kg: float, diameter_m: float) -> float:
        ...
        required_impulse = (target_altitude_m / 100.0) * base_impulse_per_100m * mass_factor * drag_factor
    ...
    def _select_motor(self, required_impulse: float, preference: Optional[str] = None, 
                     designation: Optional[str] = None, manufacturer: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    def _calculate_comprehensive_mass(self, config: Dict, requirements: RocketRequirements) -> Dict:
        ...
        # Material densities
        body_density = self.MATERIAL_DENSITIES.get(config.get('body_material', 'Fiberglass'), 1850.0)
        nose_density = self.MATERIAL_DENSITIES.get(config.get('nose_material', 'Fiberglass'), 1850.0)
        fin_density = self.MATERIAL_DENSITIES.get(config.get('fin_material', 'Fiberglass'), 1850.0)
        mount_density = self.MATERIAL_DENSITIES.get(config.get('motor_mount_material', 'Fiberglass'), 1850.0)
        ...
        main_chute_packed_vol = math.pi * (main_chute_packed_dia / 2.0)**2 * main_chute_packed_length
        ...
        drogue_chute_packed_vol = math.pi * (drogue_chute_packed_dia / 2.0)**2 * drogue_chute_packed_length
        mass_breakdown['drogue_parachute_volume'] = drogue_chute_packed_vol if config.get('has_drogue', False) else 0.0
        ...
        mass_breakdown['recovery_system'] = (
            main_chute_mass + drogue_chute_mass + 
            shock_cord_mass + deployment_bag_mass + recovery_hardware_mass
        )
        
        # 8. Avionics bay (if dual deploy)
        if config.get('has_drogue', False):
            avionics_bay_length = 0.15  # 15cm avionics bay
            avionics_bay_vol = math.pi * avionics_bay_length * (body_radius**2 - (body_radius - 0.002)**2)
            mass_breakdown['avionics_bay'] = avionics_bay_vol * body_density + 0.1  # +100g for electronics
        else:
        ...
        payload_bay_vol = math.pi * payload_bay_length * (body_radius**2 - (body_radius - 0.002)**2)
        ...
        structural_mass = (
            mass_breakdown['nose_cone'] + mass_breakdown['body_tube'] + 
            mass_breakdown['fins'] + mass_breakdown['motor_mount_tube'] +
            mass_breakdown['centering_rings'] + mass_breakdown['bulkheads']
        )
    ...
    def _estimate_dry_mass(self, nose_length: float, body_length: float, body_radius: float,
                          fin_span: float, fin_root: float, fin_tip: float,
                          payload_mass: float, fin_count: int = 4) -> float:
    ...
    def _size_main_parachute_for_distance(self, mass_kg: float, max_distance_m: float, apogee_m: float) -> float:
    ...
    def build_rocket_config(self, requirements: RocketRequirements) -> Tuple[Dict, Optional[Dict]]:
        ...
        config['manufacturing_specs'] = self._generate_manufacturing_specs(config, design.get('motor'))
    ...
    def _generate_bom(self, config: Dict, motor: Optional[Dict], mass_breakdown: Dict) -> List[Dict]:
        ...
        space_claims = {
            'component_positions': {
                ...
                'avionics_bay': {
                    ...
                    'notes': 'Altimeter, battery, deployment charges (dual deploy only)' if has_drogue else 'Not used (single deploy)'
                },
                'motor_mount': {
                    'start': motor_start_x,
                ...
                'main_parachute': {
                    ...
                    'packed_volume': math.pi * ((main_chute_dia / 25.0) / 2.0)**2 * ((main_chute_dia / 25.0) * 2.0),
                ...
                'drogue_parachute': {
                    ...
                    'packed_diameter': drogue_chute_dia / 25.0 if has_drogue else 0.0,  # Realistic packing ratio
                    'packed_length': (drogue_chute_dia / 25.0) * 2.0 if has_drogue else 0.0,
                    'packed_volume': math.pi * ((drogue_chute_dia / 25.0) / 2.0)**2 * ((drogue_chute_dia / 25.0) * 2.0) if has_drogue else 0.0,
                    'start': avionics_start_x + 0.05 if has_drogue else None,
                    'end': avionics_start_x + 0.05 + ((drogue_chute_dia / 25.0) * 2.0) if has_drogue else None,
                    'deployment_altitude': config.get('drogue_deployment_altitude', 0.0),
                    'deployment_event': config.get('drogue_deployment_event', 'APOGEE'),
                    'clearance_required': f"5 mm minimum clearance around packed chute ({drogue_chute_dia/25.0*1000:.1f} mm diameter)" if has_drogue else 'N/A',
                    'notes': f"Packed in avionics bay, deploys at apogee" if has_drogue else 'Not used (single deploy)'
                },
                'centering_rings': {
                    'positions': [
                        motor_start_x,
            ...
            'volume_claims': {
                ...
                'avionics_bay_volume': f"{math.pi * (body_radius - 0.002)**2 * avionics_length:.4f} m³" if has_drogue else "0.0 m³",
                'motor_mount_volume': f"{math.pi * motor_mount_radius**2 * motor_mount_length:.4f} m³",
                'motor_volume': f"{math.pi * (motor_diameter/2.0)**2 * motor_length:.4f} m³" if motor else "0.0 m³",
            }
        }
