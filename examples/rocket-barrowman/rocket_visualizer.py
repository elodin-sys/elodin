...
def visualize_rocket_3d(config: Dict[str, Any], motor: Optional[Dict] = None) -> go.Figure:
    ...
    body_cylinder = _generate_cylinder_points(
        start_x=nose_length, length=body_length, radius=body_radius, segments=32
    )
    fig.add_trace(
        go.Mesh3d(
            x=body_cylinder["x"],
    ...
    motor_cylinder = _generate_cylinder_points(
        start_x=motor_start_x, length=motor_mount_length, radius=motor_mount_radius, segments=16
    )
    fig.add_trace(
        go.Mesh3d(
            x=motor_cylinder["x"],
    ...
    if config.get("has_main_chute", False):
        ...
        chute_cylinder = _generate_cylinder_points(
            start_x=chute_x, length=chute_length, radius=packed_dia / 2.0, segments=16
        )
        fig.add_trace(
            go.Mesh3d(
                x=chute_cylinder["x"],
...
def _generate_cylinder_points(
    start_x: float, length: float, radius: float, segments: int = 32
) -> Dict:
...
def _generate_ring_points(
    x: float, outer_radius: float, inner_radius: float, thickness: float
) -> Dict:
...
def visualize_rocket_2d_side_view(
    config: Dict[str, Any], motor: Optional[Dict] = None
) -> go.Figure:
    """
    ...
    x_mount = [
        motor_start_x,
        motor_start_x + motor_length + 0.1,
        motor_start_x + motor_length + 0.1,
        motor_start_x,
    ]
    y_mount = [
    ...
    if motor:
        x_motor = [
            motor_start_x + 0.05,
            motor_start_x + 0.05 + motor_length,
            motor_start_x + 0.05 + motor_length,
            motor_start_x + 0.05,
        ]
