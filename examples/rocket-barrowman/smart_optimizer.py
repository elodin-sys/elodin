...
class CostBreakdown:
    ...
    def total(self) -> float:
        return sum([
            self.motor_cost, self.body_tube_cost, self.nose_cone_cost,
            self.fins_cost, self.motor_mount_cost, self.recovery_cost,
            self.hardware_cost, self.avionics_cost
        ])
...
class SmartOptimizer:
    ...
    MOTOR_COSTS = {
        "A": 6, "B": 8, "C": 10, "D": 12, "E": 18, "F": 25,
        "G": 40, "H": 65, "I": 95, "J": 140, "K": 200,
        "L": 280, "M": 400, "N": 600, "O": 900,
    }
    ...
    def __init__(self, motor_database: List["MotorData"] = None, openai_api_key: str = None):
        ...
        self.motors_by_impulse = sorted(
            [m for m in self.motors if m.total_impulse > 0],
    ...
    def _estimate_dry_mass(
        ...
        total = tube_mass + nose_mass + fin_mass + mount_mass + recovery_mass + hardware_mass + payload_mass
        return total
    ...
    def _build_and_simulate(
        ...
        nose = NoseCone(
            name="Nose", length=nose_length, base_radius=body_radius,
            thickness=0.003, shape=NoseCone.Shape.VON_KARMAN
        )
        nose.material = MATERIALS["Fiberglass"]
        rocket.add_child(nose)
        
        body = BodyTube(
            name="Body", length=body_length, outer_radius=body_radius, thickness=0.003
        )
        ...
        fins = TrapezoidFinSet(
            name="Fins", fin_count=4, root_chord=fin_root, tip_chord=fin_tip,
            span=fin_span, sweep=fin_root * 0.3, thickness=0.004
        )
        ...
        # Motor mount
        mount = InnerTube(
            name="Mount", length=motor.length + 0.05,
            outer_radius=motor.diameter/2 + 0.005, thickness=0.003
        )
        ...
        dry_mass = self._estimate_dry_mass(
            tube, body_length, nose_length, motor,
            requirements.payload_mass_kg, requirements.recovery_type
        )
        ...
        try:
            ...
            solver = FlightSolver(
                rocket=rocket_model, motor=motor_wrapped, environment=env,
                rail_length=3.0, inclination_deg=5.0, dt=0.02
            )
    ...
    def optimize(
        ...
        viable_motors = [
            m for m in self.motors_by_impulse
        ...
        for motor in viable_motors:
            ...
            if requirements.max_budget_usd and cost.total > requirements.max_budget_usd:
                log.append(f"  [{iteration}] {motor.designation}: Skip (${cost.total:.0f} > budget)")
                continue
        ...
        if best:
            ...
            log.append(f"   - Other: ${best.cost.total - best.cost.motor_cost - best.cost.body_tube_cost:.0f}")
        else:
            log.append("❌ No design found within tolerance and budget")
            if designs:
                closest = min(designs, key=lambda d: abs(d.simulated_altitude_m - target))
                log.append(f"   Closest: {closest.motor_designation} at {closest.simulated_altitude_m:.0f}m")
