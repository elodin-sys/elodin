...
FinDeflect4 = ty.Annotated[
    jax.Array,
    el.Component(
        "fin_deflect_4",
        el.ComponentType(el.PrimitiveType.F64, (4,)),
        metadata={"element_names": "fin0,fin1,fin2,fin3"},
    ),
]
...
class Rocket(el.Archetype):
    ...
    fin_deflect_4: FinDeflect4 = field(default_factory=lambda: jnp.array([0.0, 0.0, 0.0, 0.0]))
...
@el.system
def fin_deflect_4_sinusoidal(
    tick: el.Query[el.SimulationTick],
    dt: el.Query[el.SimulationTimeStep],
    q: el.Query[FinDeflect4],
) -> el.Query[FinDeflect4]:
    """
    Generate sinusoidal fin deflections with phase shifts.
    Each fin gets a different phase offset to make them distinct.
    """
    t = tick[0] * dt[0]
    ...
    amplitude = 20
    # Phase offsets for each fin (in radians)
    phases = jnp.array([0.0, jnp.pi / 2, jnp.pi, 3 * jnp.pi / 2])
    # Generate sinusoidal values for each fin
    fin_values = amplitude * jnp.sin(2 * jnp.pi * frequency * t + phases)
    return q.map(FinDeflect4, lambda _: fin_values)
    ...
    hsplit {
        ...
        vsplit share=0.4 {
            vsplit {
                ...
                graph "rocket.fin_deflect_4" name="Fin Deflection"
            }
            vsplit {
