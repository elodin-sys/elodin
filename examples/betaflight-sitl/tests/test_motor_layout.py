...
def test_native_motor_order_positions_and_spins() -> None:
    ...
    np.testing.assert_allclose(np.linalg.norm(config.motor_positions, axis=1), config.arm_length)
...
def test_individual_motor_torque_signs(motor_index: int, expected_torque_signs) -> None:
...
def test_equal_motor_thrust_cancels_body_torque() -> None:
    ...
    yaw_torque = config.motor_torque_coeff * np.dot(equal_thrust, config.motor_spin_directions)
