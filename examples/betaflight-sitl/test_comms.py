...
import numpy as np
from comms import (
    BetaflightBridge, FDMPacket, RCPacket,
    ...
    try:
        while running:
            ...
            if current_time - last_print_time >= 1.0:
                ...
                print(f"  Motors (BF order):  [{motors[0]:.3f}, {motors[1]:.3f}, "
                      f"{motors[2]:.3f}, {motors[3]:.3f}]")
                print(f"  Motors (Elodin):    [{motors_elodin[0]:.3f}, {motors_elodin[1]:.3f}, "
                      f"{motors_elodin[2]:.3f}, {motors_elodin[3]:.3f}]")
                print(f"  Raw PWM (first 4):  [{motors_raw[0]:.0f}, {motors_raw[1]:.0f}, "
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Test Betaflight SITL UDP communication"
    )
    parser.add_argument(
        "--arm", action="store_true",
        help="Run arming sequence test"
    )
    parser.add_argument(
