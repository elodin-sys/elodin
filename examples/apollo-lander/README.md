...
powered-descent problem. The simulated Lunar Module descends under lunar gravity
and a throttleable descent engine, while an external controller process plays a
minimal Lunar Guidance Computer (LGC): it receives telemetry over UDP and sends
back a throttle command.

The example also replays real Apollo 11 LM telemetry as a truth vehicle. In the
editor, the simulated LM and real LM are drawn together over the landing-site
asset so the campaign has both a flight-control goal and a visual story.
...
- A full 6-DOF `el.six_dof` simulation: lunar gravity, a throttleable body-axis
  descent engine, RCS attitude control, and propellant burn-down updating mass.
  All physics lives in JAX systems; the `post_step` callback only reads
  kinematics and writes external-control commands (throttle + attitude setpoint).
...
- A real KDL schematic: LM GLB, a green "truth" LM GLB, landing-site GLB,
  ...
  graphs for altitude, descent rate, horizontal speed, pitch, throttle, and
  propellant.
...
- Monte Carlo robustness scoring: soft landing if vertical touchdown speed is
  `<= 3 m/s`, horizontal speed `<= 1 m/s`, the LM is near-upright, and fuel remains.
- Monte Carlo calibration: each run reports altitude RMSE against the real
  descent profile so you can iteratively narrow parameter ranges toward the real
  landing.
...
- `lunar_gravity` applies `-g * mass` (world up).
...
`~8,212 kg` descent propellant). The telemetry window opens at ~13 km, after
the braking phase had already burned roughly half of the DPS load, so the
simulation defaults to `~4,100 kg` of propellant remaining at window start
(`~11,200 kg` wet). The descent engine is modeled with the published throttle
range (`4,670-45,040 N`) and `Isp ~= 311 s`. RCS is modeled as 16 x `445 N`
thrusters with a representative moment arm and `Isp ~= 290 s`; RCS propellant
is tracked separately and included in the vehicle mass.

The external LGC flies the Apollo descent as a reference-trajectory follower: it
tracks the smoothed altitude / descent-rate profile derived from the mission
telemetry while nulling horizontal velocity, and emits a throttle plus an
attitude-setpoint quaternion. The commanded thrust direction is constrained to
an upward cone so guidance never asks the LM to flip over; if it needs to fall
faster, it throttles down rather than inverting the vehicle. The in-sim RCS loop
points the vehicle every tick.

## Real Telemetry

Two truth sources live in `data/`:

- `apollo11_lem_raw.csv` - the verbatim public telemetry from
  https://github.com/jumpjack/Apollo11LEMdata/blob/master/data.csv
  (NASA postflight material: IMU stable-member gimbal angles plus `RANGE (FT)`).
- `apollo11_descent.csv` - the derived, SI-unit measurements (relative time,
  range in meters, gimbal angles) used by the simulation.

`reference.py` loads the derived data, rejects radar-range spikes with a median
filter, smooths to a monotonic descent, removes the terminal antenna-height
offset (the radar reads ~4 m at touchdown), and builds the altitude /
descent-rate / pitch reference shared by the truth display and the controller.
It also provides
`sanity_check()`, which confirms the derived measurements agree with the raw
GitHub source (run `python reference.py` to print the profile and the check).

`RANGE` is treated as an altitude proxy, and the truth pitch is an approximate
trend from the inner gimbal angle (a full vehicle attitude needs the mission
REFSMMAT, which the dataset does not include). The green truth LM therefore
matches the recorded descent profile in altitude; its attitude and the reported
`pitch_rmse` are illustrative, not an exact attitude reconstruction.
...
margin distribution, and the best-fit run by minimum trajectory RMSE.
