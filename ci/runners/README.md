...
- **Windows `Post-build` fails with "stream did not contain valid UTF-8":** `pwsh` 7 is missing, so the `Build artifacts` step (no `shell:`) ran under PowerShell 5.1 and wrote `dist-manifest.json` as UTF-16. `choco install powershell-core --yes`, then `Restart-Service actions.runner.*` so the service sees the new PATH.
