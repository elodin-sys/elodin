...
with pkgs; let
  xla_ext = callPackage ./pkgs/xla-ext.nix {};
in {
  c = (mkShell.override {stdenv = llvmPackages_20.libcxxStdenv;}) {
    name = "elo-c-shell";
    buildInputs = [];
    doCheck = false;
  };
  rust = mkShell {
    ...
    DYLD_LIBRARY_PATH = lib.optionalString stdenv.isDarwin "${xla_ext}/lib";
  };
  ops = mkShell {
    name = "elo-ops-shell";
    buildInputs = [
