...
MB2 initializes every device listed in its cold-boot BCT storage info. On the
Aleph carrier PCIe cannot come up that early: MB2 dies with an SError in
    ...
    def aleph_storage_pt(self):
        ...
        if not os.path.exists(out_bin):
            with open(values['--cfg']) as f:
                xml_txt = f.read()
            xml_txt = re.sub(r'<device[^>]*type="nvme"[\\s\\S]*?</device>', '', xml_txt)
...
def main() -> None:
    ...
    if count == 0:
        raise SystemExit("updatestorageinfo pattern not found")
