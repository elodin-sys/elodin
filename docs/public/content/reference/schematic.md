...
- `video_stream`: positional `msg_name` (required) — the message name used by the video source (e.g. `elodinsink --msg-name` or RTMP stream key). `name` (optional display label, defaults to "Video Stream"). Displays live H.264 video from the named message log. Example: `video_stream "camera-1" name="Front Camera"`.
