...
- `earth` child (optional): the editor's built-in cinematic Earth for **ECEF** scenes. One word spawns everything from assets embedded in the editor binary: a true-scale WGS84 globe at the ECEF origin (8K day/night/cloud textures), a raymarched atmosphere, the Milky Way skybox, a GPU-particle star field, night city lights, and green/red airglow shells, plus earthshine on the craft. Haze density, star/skybox visibility, city-light intensity, and airglow follow the cinematic camera and sun, so one schematic works from the launch pad to LEO.
  ...
  - Parse errors (not warnings): more than one `cinematic=#true` viewport; any `hdr=#true` viewport in the same schematic as a cinematic viewport; `earth` without a cinematic viewport; `cinematic=#true` without `earth`. Validation is per KDL document (multi-window schematics are not cross-checked). Do **not** mix a cinematic viewport with `world.sensor_camera(cinematic=True)` — only one cinematic environment owner is supported (Bevy 0.19 can attach atmosphere to a single view). Prototype look on a cinematic viewport, then move `environment` / `ev100` / `bloom` onto the sensor camera and strip `cinematic=#true` and `environment { earth }` from the schematic.
...
- `orientation`: optional; `relative` (default) or `absolute`. `relative` re-expresses the attitude as a rotation operator in Bevy (`bevy_R * att * bevy_R⁻¹`); identity attitude leaves the mesh in its authored Bevy / glTF axes (Y-up). `absolute` composes with the frame→Bevy basis (`bevy_R * att`); identity attitude aligns mesh local axes with the schematic frame (ENU: +X east, +Y north, +Z up). Use `absolute` for frame-aligned geometry (compasses, ECEF vehicles). Body-frame EQL offsets (`.translate`, `.translate_x`/`.translate_y`/`.translate_z`, `.direction`) default to the **absolute** sense so a chase camera lands on the rendered axis of a default-relative mesh. Pass a trailing `true` — e.g. `.translate(-2, 0, 0, true)` or `.direction(0, 0, 1, true)` — for the **relative** sense (`att · offset`). `false` equals the default. In ENU the two coincide; cameras chasing `orientation=absolute` objects in ECEF/NED should pass `true`. The pose's frame comes from the compile site (`viewport` / `vector_arrow` / `object_3d` `frame=`).
...
- Mesh child (required, exactly one):
  ...
  - `plane`: `width`/`depth` (default `size` if set, else 10.0); optional `size` shorthand; `color` (default white). Horizontal ground rectangle in Bevy axes (XZ, normal +Y).
...
viewport = "viewport"
         ...
         [frustums_up_marker=none|highlight]
