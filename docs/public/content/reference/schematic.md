...
- `spatial_gauge`: positional `eql` (required position expression; named `eql=` also accepted), `name` (optional), `source` (`ECEF`/`NED`/`ENU`; inherits from global `coordinate` if omitted, else `ENU`), `display` (`ECEF`/`NED`/`ENU`/`LLA`, default `NED`), optional `reference x y z w` child (body→`source` quaternion shown as neutral by the attitude gimbal; identity when omitted). Reads a position in `source` and shows it converted to `display`.
