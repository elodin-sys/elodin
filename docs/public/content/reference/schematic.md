...
- `spatial_gauge`: `eql` (required position expression), `name` (optional), `source` (`ECEF`/`NED`/`ENU`, default `ECEF`), `display` (`ECEF`/`NED`/`ENU`/`LLA`, default `NED`). Reads a position in `source` and shows it converted to `display`.
