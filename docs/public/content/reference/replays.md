...
For **Elodin DB** recordings that bundle schematic assets (GLB, `.png` icons, skyboxes) under `{db}/assets/`, see [DB Asset Server](/reference/db-asset-server). The layout below describes the legacy replay directory format (`metadata.json`, `assets.bin`, Parquet).
